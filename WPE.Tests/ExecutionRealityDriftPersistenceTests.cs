...
public sealed class ExecutionRealityDriftPersistenceTests : IDisposable
{
    ...
    {
        var fact = Fact("strategy-a", "v1", "order-a", "FILLED", 1m, 100.2m, .04008m, IntendedAt.AddSeconds(1));
    ...
    [Fact]
    public async Task SummarySeparatesComparableExecutionFromTerminalNoFill()
    {
        var store = new AgentSqliteStore(Database);
        await store.SaveExecutionRealityDriftAsync(
            Fact("strategy-a", "v1", "filled", "FILLED", 1m, 100.2m, .04008m, IntendedAt.AddSeconds(1)),
            default);
        await store.SaveExecutionRealityDriftAsync(
            Fact("strategy-a", "v1", "partial", "PARTIALLY_FILLED", .5m, 99.9m, .01998m, IntendedAt.AddSeconds(2)),
            default);
        await store.SaveExecutionRealityDriftAsync(
            Fact("strategy-a", "v1", "rejected", "REJECTED", 0m, 0m, 0m, IntendedAt.AddMilliseconds(500)),
            default);
        await store.SaveExecutionRealityDriftAsync(
            Fact("strategy-b", "v7", "other", "FILLED", 1m, 110m, .044m, IntendedAt.AddSeconds(3), expectedPrice:110m),
            default);
        ...
        Assert.Equal(2, summary.ComparableCount);
        Assert.Equal(2, summary.TerminalCount);
        Assert.Equal(.75m, summary.AverageFillRatio);
        Assert.Equal(-5m, summary.AverageSlippageDriftBps);
        Assert.Equal(0m, summary.AverageFeeDriftBps);
        Assert.Equal(-5m, summary.AverageTotalExecutionDriftBps);
        Assert.Equal(2000, summary.MaximumObservationLatencyMs);
        Assert.Equal(IntendedAt.AddSeconds(2), summary.LatestObservedAtUtc);
    ...
    {
        var store = new AgentSqliteStore(Database);
        var fact = Fact("strategy-a", "v1", "order-a", "FILLED", 1m, 100m, .04m, IntendedAt.AddSeconds(1));
