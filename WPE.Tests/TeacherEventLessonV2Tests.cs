...
public sealed class TeacherEventLessonV2Tests:IDisposable
{
    ...
    [Fact] public void ProductionTradingLoopDoesNotStartTeacherNetwork()
    {
        var source=File.ReadAllText(Path.Combine(ProjectRoot(),"Services","AutoTradingAgent.cs"));
        Assert.DoesNotContain("TeacherCryptoEvidenceSchedulerV2",source,StringComparison.Ordinal);
        Assert.DoesNotContain("teacherCryptoSchedulerTask",source,StringComparison.Ordinal);
    }
