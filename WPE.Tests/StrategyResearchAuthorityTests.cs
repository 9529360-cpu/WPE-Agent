...
public sealed class StrategyResearchAuthorityTests : IDisposable
{
    ...
    public async Task LegacyValidationWithoutCurrentTemporalPolicyCannotAuthorize()
    {
        ...
        await store.UpsertStrategyAsync(profile,CancellationToken.None);
        var legacy=Validation(profile,true) with { OosPurgeObservations=0,OosEmbargoObservations=0 };
    ...
    private static StrategyValidation Validation(StrategyProfile profile,bool passed)=>new(
        ...
        HistoricalResearchEngine.PromotionTemporalPolicy.OosPurgeObservations,
        HistoricalResearchEngine.PromotionTemporalPolicy.OosEmbargoObservations);
