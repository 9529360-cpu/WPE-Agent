...
public partial class SetupWindow:Window
{
    ...
    private async Task CheckAll(){SaveExchange();SaveAdvanced();HealthText.Text=I18n.T("Setup.Checking");_last=await _readiness.CheckAsync(_settings);HealthText.Text=string.Join("\n",_last.Checks.Select(SafeCheck));_settings.LastAccessCheckAtUtc=_last.CheckedAtUtc;_store.Save(_settings);}
    ...
    private async void TestBrain_Click(object sender,RoutedEventArgs e){try{var report=await _readiness.CheckAsync(_settings);var b=report.Checks.First(x=>x.Key=="local_brain");BrainStatus.Text=SafeCheck(b);}catch(Exception ex){BrainStatus.Text=UiDiagnostic.Format(ex,"Local brain check failed.");}}
