...
public partial class SetupWindow:Window
{
    ...
    private void SaveBrain(){_settings.AiMode=币安量化机器人.Core.Models.AiRuntimeMode.LocalOnly;AgentSettingsStore.ValidateBrainEndpoint(RuntimeModePolicy.GetActiveBrain(_settings));_store.Save(_settings);}
    ...
    private async void TestBrain_Click(object sender,RoutedEventArgs e){try{SaveBrain();var report=await _readiness.CheckAsync(_settings);var b=report.Checks.First(x=>x.Key=="local_brain");BrainStatus.Text=SafeCheck(b);}catch(Exception ex){BrainStatus.Text=UiDiagnostic.Format(ex,"Local brain check failed.");}}
