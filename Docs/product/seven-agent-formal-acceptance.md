...
Each aggregate Agent remains `partial` until its own four required evidence classes pass together. Component acceptance, a passing unit test, a live process, or a public price response cannot promote an aggregate. Market, Research, and Strategy are formally accepted only within their exact candidate-bound scopes recorded in the maturity authority; the other four aggregates remain `partial`.
...
Risk local evidence is produced by two independent focused TRX runs and packaged with `--risk-local-acceptance`. Exact allowlists bind deterministic rule/intent hashes, stale/incomplete/account limit rejection, Mainnet rejection, the shared authorization gateway, replay rejection, and zero-mutation behavior under bypass attempts. These artifacts do not satisfy `risk.live-preauthorization` or `risk.toctou-revalidation`; Risk remains `partial` until both Testnet artifacts use authoritative account/position/capability facts from the same candidate.
