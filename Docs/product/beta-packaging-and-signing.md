...
The output directory contains the portable ZIP, `SHA256SUMS`, and `package-result.json`. Inside the ZIP, `app/`, `headless/`, and `maintenance/` are the three runtime roots; `RELEASE-METADATA.json` records version/channel/architecture, source identity, runtime tree hashes and signing provenance; `RELEASE-READINESS.json` preserves the exact readiness report; a signed bundle also contains `SIGNING-RESULT.json`; `FILE-MANIFEST.json` and `PAYLOAD-SHA256SUMS` cover every file under all three runtime roots using package-root-relative paths; `sbom.cdx.json` is a CycloneDX 1.5 inventory generated from the Desktop, Headless, Maintenance NuGet graphs plus the pnpm production dependency graph. Unknown licenses remain `NOASSERTION`; the script does not guess.

An unsigned package is permitted only when all three executables are NotSigned and all three runtime trees still exactly match release-readiness bytes; it is internal-evaluation-only and non-distributable. A signed package requires all three executables to be Valid, one approved signer subject/thumbprint, a valid readiness-to-signing transition record, clean source, and no mixed signing state.
