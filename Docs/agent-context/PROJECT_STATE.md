...
Known gaps: target-machine live availability evidence for the official BLS release calendar, protocol/tokenomics/on-chain Fundamental evidence beyond verified Binance Testnet instrument metadata, funding attribution, broader causal post-trade strategy attribution, and fee/fundamental certification for providers other than Binance Futures Testnet. Provider-neutral intent-versus-fill slippage attribution is now persisted, but venue-specific latency and market-impact decomposition remain open. Legacy Binance-direct execution services are excluded from production compilation and guarded by regression tests.
