...
public static class DeviceLicenseTestRunner
{
    ...
    public static async Task<SmokeRunResult> RunAsync(CancellationToken ct=default)
    {
        ...
        try
        {
            ...
            await Case(report,"篡改激活码无法通过签名验证",()=>{var signatureStart=code.LastIndexOf('.')+1;var replacement=code[signatureStart]=='A'?'B':'A';var tamperedCode=code[..signatureStart]+replacement+code[(signatureStart+1)..];var tampered=DeviceLicenseCodec.Verify(tamperedCode,publicKey,device,DateTime.UtcNow);Require(!tampered.Success,"篡改码被接受");return tampered.Message;});
