...
public sealed class LongHorizonResearchSkill
{
    ...
    public ResearchValidationResult Evaluate(string symbol,IReadOnlyList<CandleEvidence> candles,RiskLimits limits,string version="wpe-hourly-v3")
    {
        if(candles.Count<500)return new(){Symbol=symbol,StrategyVersion=version,SampleSize=candles.Count,CoverageDays=Coverage(candles),Approved=false,Promoted=false,Summary=$"{symbol} historical coverage insufficient: {candles.Count} hourly candles"};
...
public sealed class PortfolioRiskSkill
{
    ...
    {
        var equity=evidence.Account.Equity;if(equity<=0)return new(){Approved=false,BlockingReasons=["account_equity_unavailable"],Summary="Portfolio risk unavailable: account equity is zero"};var exposure=new Dictionary<string,decimal>(StringComparer.OrdinalIgnoreCase);foreach(var p in evidence.Positions)exposure[p.Symbol]=exposure.GetValueOrDefault(p.Symbol)+(p.Side==PositionSide.Long?1:-1)*p.Quantity*p.MarkPrice;foreach(var i in intents){var price=i.ExpectedPrice>0?i.ExpectedPrice:evidence.Markets.GetValueOrDefault(i.Symbol)?.Price??0;var change=i.Quantity*price*(i.Side==PositionSide.Long?1:-1)*(i.ReduceOnly?-1:1);exposure[i.Symbol]=exposure.GetValueOrDefault(i.Symbol)+change;}exposure=exposure.Where(x=>Math.Abs(x.Value)>0).ToDictionary();if(exposure.Count==0)return new(){Approved=true,Summary="Portfolio flat; VaR and concentration are zero"};var gross=exposure.Values.Sum(Math.Abs);var net=Math.Abs(exposure.Values.Sum());var largest=(double)(exposure.Values.Max(Math.Abs)/gross);var returns=history.ToDictionary(x=>x.Key,x=>x.Value.Skip(1).Select((c,i)=>x.Value[i].Close>0?(double)(c.Close/x.Value[i].Close-1):0).TakeLast(4000).ToArray(),StringComparer.OrdinalIgnoreCase);var correlations=new Dictionary<string,double>();var symbols=exposure.Keys.Order().ToArray();var maxCorr=0d;for(var a=0;a<symbols.Length;a++)for(var b=a+1;b<symbols.Length;b++){var corr=Correlation(returns.GetValueOrDefault(symbols[a])??[],returns.GetValueOrDefault(symbols[b])??[]);correlations[$"{symbols[a]}:{symbols[b]}"]=corr;maxCorr=Math.Max(maxCorr,Math.Abs(corr));}var count=symbols.Select(x=>returns.GetValueOrDefault(x)?.Length??0).DefaultIfEmpty().Min();var portfolio=new List<double>();for(var n=0;n<count;n++){var value=0d;foreach(var symbol in symbols){var series=returns.GetValueOrDefault(symbol)??[];value+=(double)(exposure[symbol]/equity)*series[series.Length-count+n];}portfolio.Add(value);}var var95=QuantileLoss(portfolio,.95);var var99=QuantileLoss(portfolio,.99);var cvar=portfolio.Count==0?0:portfolio.Where(x=>-x>=var99).Select(x=>-x).DefaultIfEmpty(var99).Average();var stress=(double)(gross/equity)*.15;var blocks=new List<string>();if(var99>limits.MaxPortfolioVaR99)blocks.Add($"VaR99 {var99:P2}>{limits.MaxPortfolioVaR99:P2}");if(cvar>limits.MaxPortfolioCVaR99)blocks.Add($"CVaR99 {cvar:P2}>{limits.MaxPortfolioCVaR99:P2}");if(largest>limits.MaxLargestPositionShare&&exposure.Count>1)blocks.Add($"concentration {largest:P1}>{limits.MaxLargestPositionShare:P1}");foreach(var pair in correlations.Where(x=>Math.Abs(x.Value)>.85)){var names=pair.Key.Split(':');var combined=(Math.Abs(exposure[names[0]])+Math.Abs(exposure[names[1]]))/equity;if((double)combined>limits.MaxCorrelatedExposure)blocks.Add($"correlated exposure {pair.Key} {combined:P1}");}return new(){GrossExposure=gross,NetExposure=net,LargestPositionShare=largest,VaR95=var95,VaR99=var99,CVaR99=cvar,StressLoss=stress,MaximumPairCorrelation=maxCorr,Correlations=correlations,BlockingReasons=blocks,Approved=blocks.Count==0,Summary=$"gross={gross/equity:P1} net={net/equity:P1} largest={largest:P1} VaR99={var99:P2} CVaR99={cvar:P2} stress={stress:P1} corr={maxCorr:F2} approved={blocks.Count==0}"};
    }
