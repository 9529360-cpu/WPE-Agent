...
public sealed record RiskLocalAcceptanceRunResult(bool Success,string ArtifactDirectory,IReadOnlyList<string> Errors);

public static class RiskLocalAcceptanceRunnerV1
{
    public static async Task<RiskLocalAcceptanceRunResult> RunAsync(string[] args,CancellationToken ct=default)
    {
        var artifactsRoot=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"WPE Agent","Runtime","TestArtifacts");var directory=Path.Combine(artifactsRoot,"risk-local-acceptance",DateTime.UtcNow.ToString("yyyyMMdd-HHmmss")+"-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(directory);var errors=new List<string>();var files=new List<object>();var candidateVersion="unknown";var candidateHash="";var testHash="";
        try
        {
            var options=Parse(args);var gatePath=Required(options,"deterministic-gate-trx");var bypassPath=Required(options,"adversarial-bypass-trx");var testAssemblyPath=Required(options,"test-assembly");var assembly=Assembly.GetExecutingAssembly();var candidateBytes=await File.ReadAllBytesAsync(assembly.Location,ct);var testBytes=await File.ReadAllBytesAsync(testAssemblyPath,ct);candidateVersion=assembly.GetName().Version?.ToString()??"unknown";candidateHash=Hash(candidateBytes);testHash=Hash(testBytes);
            foreach(var input in new[]{("risk.deterministic-gate",gatePath),("risk.adversarial-bypass",bypassPath)}){var trx=await File.ReadAllBytesAsync(input.Item2,ct);var artifact=LocalTestAcceptanceCanonicalizerV1.Create(input.Item1,candidateVersion,candidateBytes,testBytes,trx);if(!LocalTestAcceptanceCanonicalizerV1.IsCanonical(artifact,DateTimeOffset.UtcNow,candidateHash))throw new InvalidOperationException($"Generated local evidence is ineligible: {input.Item1}");var name=input.Item1+".json";await AtomicWriteAsync(Path.Combine(directory,name),artifact.CanonicalBytes,ct);files.Add(new{name,artifact.RequirementId,artifact.CanonicalSha256,artifact.ObservedAtUtc,tests=artifact.Tests.Count});}
        }
        catch(OperationCanceledException)when(ct.IsCancellationRequested){throw;}
        catch(Exception ex){errors.Add("risk-local-acceptance.failed:"+ex.GetType().Name);}
        var manifest=JsonSerializer.SerializeToUtf8Bytes(new{schema="wpe.risk-local-acceptance-run/1.0",candidateVersion,candidateSha256=candidateHash,testAssemblySha256=testHash,completedAtUtc=DateTimeOffset.UtcNow,success=errors.Count==0,files,errors});await AtomicWriteAsync(Path.Combine(directory,"manifest.json"),manifest,ct);await AtomicWriteAsync(Path.Combine(directory,"manifest.sha256"),System.Text.Encoding.ASCII.GetBytes(Hash(manifest)),ct);return new(errors.Count==0,directory,errors);
    }
