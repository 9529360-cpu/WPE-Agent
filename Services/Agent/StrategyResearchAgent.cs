...
{
    internal static readonly ResearchTemporalIsolationPolicy PromotionTemporalPolicy=new(.65,30,5,5);
    ...
    public StrategyValidation Validate(StrategyProfile profile, IReadOnlyList<CandleEvidence> candles, IReadOnlyList<NewsFeature> news, RiskLimits limits)
    {
        ...
        var temporalPoints=timeline.Select(x=>new ResearchTemporalPoint(x.TradableAtUtc,x.EvidenceAvailableAtUtc,x.SignalGeneratedAtUtc)).ToArray();
        var temporalReasons=ResearchTemporalValidationV1.Validate(temporalPoints,returns.Count,PromotionTemporalPolicy);
        if(temporalReasons.Count>0)return new(profile.Id,candles.Count,0,0,0,0,1,0,0,0,1,0,false,$"temporal research gate failed: {string.Join(',',temporalReasons)}",StrategyVersion:profile.Version);
        var bounds=ResearchTemporalValidationV1.Bounds(returns.Count,PromotionTemporalPolicy);
        var train=returns.Take(bounds.TrainingEndExclusive).ToArray();var test=returns.Skip(bounds.OosStart).Take(bounds.OosCount).ToArray();
