...
{
    internal static readonly CrossAssetValidationPolicy ProductionTemporalPolicy=new(OosPurgeObservations:5,OosEmbargoObservations:5,MinimumOutOfSampleObservations:30);
    ...
    public StrategyValidation Validate(StrategyProfile profile, IReadOnlyList<CandleEvidence> candles, IReadOnlyList<NewsFeature> news, RiskLimits limits, IReadOnlyList<StrategyProfile>? parameterTrials=null)
    {
        ...
        var temporalReasons=ResearchTemporalIsolationV1.Validate(
            timeline.Select(x=>new ResearchTemporalObservationV1(x.TradableAtUtc,x.EvidenceAvailableAtUtc,x.SignalGeneratedAtUtc)),
            timeline.Count,ProductionTemporalPolicy);
        var returns=_reality.Simulate(timeline);
        if(returns.Count<2||temporalReasons.Count>0)
            return new(profile.Id,candles.Count,0,0,0,0,1,0,0,0,1,0,false,
                temporalReasons.Count>0?string.Join(';',temporalReasons):"canonical research timeline unavailable or invalid",
