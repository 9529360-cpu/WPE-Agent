...
public sealed class ReliableOrderExecutor
{
    ...
    public async Task<string> ExecuteAsync(string cycle,ExecutionIntent i,int leverage,bool isolated,CancellationToken ct)
    {
        ...
        await _db.SaveIntentAsync(cycle,i,order.Status,order.OrderId,ct);if(order.Status!="FILLED")throw new InvalidOperationException(L("Execution.Unconfirmed",order.Status));if(i.ReduceOnly){if(IsFullClose(i.Action)){await CancelProtectionOrdersAsync(i.Symbol,i.Side,ct);await _db.ClearLockedSideIfMatchesAsync(i.Symbol,i.Side,ct);}if(i.Action==DecisionAction.Unlock)await _db.ClearLockedSideAsync(i.Symbol,ct);await _db.SaveIntentAsync(cycle,i,"COMPLETED",order.OrderId,ct);return L("Execution.CloseConfirmed");}
        try{await _ex.PlaceProtectionAsync(i.Symbol,i.Side,i.StopLoss,i.TakeProfit,i.ClientOrderId,ct);if(i.Action==DecisionAction.Lock)await _db.SetLockedSideAsync(i.Symbol,i.Side,ct);await _db.SaveIntentAsync(cycle,i,"PROTECTED",order.OrderId,ct);return L("Execution.Protected");}
        catch
        {
            ...
            await _db.SaveIntentAsync(cycle,i,close.Status=="FILLED"?"EMERGENCY_CLOSED":"EMERGENCY_UNKNOWN",close.OrderId,ct);throw new InvalidOperationException(close.Status=="FILLED"?L("Execution.ProtectionEmergencyClosed"):L("Execution.ProtectionEmergencyUnknown",close.Status));
        }
    }
    public async Task<RecoveryResult> RecoverPendingAsync(CancellationToken ct)
    {
        ...
        {
            var i=saved.Intent;var order=await _ex.FindOrderAsync(i.Symbol,i.ClientOrderId,ct);if(order is null){await _db.SaveIntentAsync(saved.CycleId,i,"UNKNOWN",saved.ExchangeOrderId,ct);safe=false;messages.Add(L("Execution.ExchangeUnknown",i.ClientOrderId));continue;}
            await _db.SaveIntentAsync(saved.CycleId,i,order.Status,order.OrderId,ct);if(order.Status=="FILLED"&&!i.ReduceOnly)try{await _ex.PlaceProtectionAsync(i.Symbol,i.Side,i.StopLoss,i.TakeProfit,i.ClientOrderId,ct);await _db.SaveIntentAsync(saved.CycleId,i,"PROTECTED",order.OrderId,ct);messages.Add(L("Execution.ProtectionRecovered",i.Symbol,i.Side));}catch(Exception ex){safe=false;messages.Add(L("Execution.ProtectionRecoveryFailed",i.Symbol,i.Side,ex.Message));}
            else if(order.Status=="FILLED"){await _db.SaveIntentAsync(saved.CycleId,i,"COMPLETED",order.OrderId,ct);}else if(order.Status is "NEW" or "PARTIALLY_FILLED"){safe=false;messages.Add(L("Execution.Pending",i.ClientOrderId,order.Status));}
    ...
    public async Task<RecoveryResult> AuditAndRepairProtectionAsync(IReadOnlyList<ManagedPosition> positions,IReadOnlyList<ExchangeOrder> orders,CancellationToken ct)
    {
        var safe=true;var messages=new List<string>();foreach(var p in positions)
        {
            ...
            var intent=await _db.GetLatestOpeningIntentAsync(p.Symbol,p.Side,ct);if(intent is null){safe=false;messages.Add(L("Execution.ProtectionMissing",p.Symbol,p.Side,!hasSl,!hasTp));continue;}
            try{await _ex.PlaceProtectionAsync(p.Symbol,p.Side,intent.StopLoss,intent.TakeProfit,intent.ClientOrderId,ct);messages.Add(L("Execution.ProtectionRepaired",p.Symbol,p.Side));}catch(Exception ex){safe=false;messages.Add(L("Execution.ProtectionRepairFailed",p.Symbol,p.Side,ex.Message));}
        }return new(safe,messages);
    }
