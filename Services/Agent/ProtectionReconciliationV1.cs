...
public static class ProtectionReconciliationServiceV1
{
    ...
    public static ProtectionReconciliationReportV1 Reconcile(IReadOnlyList<ManagedPosition> positions,IReadOnlyList<ExchangeOrder> orders,DateTimeOffset observedAtUtc,DateTimeOffset evaluatedAtUtc)
    {
        ...
        var legs=new List<ProtectionEvidenceLegV1>();foreach(var key in positionKeys.OrderBy(x=>x,StringComparer.Ordinal)){var evidence=protection.GetValueOrDefault(key)??[];var types=evidence.Select(x=>x.Type.ToUpperInvariant()).ToArray();var combined=types.Any(x=>x.Contains("OCO",StringComparison.Ordinal)||x.Contains("POSITION_TPSL",StringComparison.Ordinal));var stop=combined||types.Any(x=>x.Contains("STOP",StringComparison.Ordinal)||x.Contains("LOSS",StringComparison.Ordinal));var take=combined||types.Any(x=>x.Contains("TAKE",StringComparison.Ordinal)||x.Contains("PROFIT",StringComparison.Ordinal));if(!stop)reasons.Add("protection.missing-stop:"+key);if(!take)reasons.Add("protection.missing-take-profit:"+key);var parts=key.Split(':');legs.Add(new(parts[0],Enum.Parse<PositionSide>(parts[1],true),stop,take,combined?"combined":stop&&take?"separate":"incomplete",evidence.Count));}
