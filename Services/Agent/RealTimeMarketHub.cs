...
public sealed class RealTimeMarketHub : IRealtimeMarketFeed
{
    ...
    internal async Task HandleMarketAsync(string json,CancellationToken ct)
    {
        ...
        {
            var accepted=false;
            if(eventType=="bookTicker"){var bid=D(data,"b");var ask=D(data,"a");var bidQuantity=D(data,"B");var askQuantity=D(data,"A");var bookAt=EventTime(data,"T")??EventTime(data,"E");if(bookAt is not null&&bookAt.Value<=receivedAt&&bookAt.Value>=state.BookUpdatedAt&&bid>0&&ask>=bid&&bidQuantity>=0&&askQuantity>=0){state.BestBid=bid;state.BestAsk=ask;state.BidQuantity=bidQuantity;state.AskQuantity=askQuantity;state.BookUpdatedAt=bookAt.Value;accepted=true;}}
