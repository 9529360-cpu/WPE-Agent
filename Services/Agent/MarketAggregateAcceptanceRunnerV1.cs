...
public static class MarketAggregateAcceptanceRunnerV1
{
    ...
    {
        var directory=Path.Combine(AppDataPaths.TestArtifactsDirectory,"market-aggregate-acceptance",DateTime.UtcNow.ToString("yyyyMMdd-HHmmss")+"-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(directory);var errors=new List<string>();var files=new List<object>();
        try
        {
            await using var source=new BinanceTestnetMarketAcceptanceSourceV1();
            var live=await MarketAggregateAcceptanceCollectorV1.CollectAsync(source,MarketAggregateEvidenceCanonicalizerV1.LiveReadRequirement,symbol,1,TimeSpan.Zero,ct:ct);
            var sustained=await MarketAggregateAcceptanceCollectorV1.CollectAsync(source,MarketAggregateEvidenceCanonicalizerV1.SustainedFreshnessRequirement,symbol,4,TimeSpan.FromMinutes(5),ct:ct);
            foreach(var artifact in new[]{live,sustained})
            {
                ...
                var name=artifact.RequirementId+".json";await AtomicWriteAsync(Path.Combine(directory,name),artifact.CanonicalBytes,ct);files.Add(new{name,artifact.RequirementId,artifact.CanonicalSha256,artifact.StartedAtUtc,artifact.CompletedAtUtc,samples=artifact.Samples.Count});
            }
        }
