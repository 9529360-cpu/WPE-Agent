...
public sealed class BinanceFuturesAdapter : IExchangeProvider,IMarketDataProvider,IBrokerProvider,IProviderMarketCatalog,IProviderEnvironmentGuard,IRecentOrderProvider,IExchangeOrderFeeEvidenceReader,IExchangeFundingIncomeReader,ICryptoInstrumentFundamentalReader
{
    ...
    private async Task<ExchangeOrder> PlaceAlgoProtectionAsync(string symbol,OrderSide side,string positionSide,string type,decimal triggerPrice,string clientAlgoId,CancellationToken ct)
    {
        ...
        var order=MapAlgo(document.RootElement) with{ProtectionCoverage=ProtectionCoverageKind.PositionWide};_algoOrderIds[order.OrderId]=0;return order;
    }
    private static bool Bool(JsonElement e,string n)=>e.TryGetProperty(n,out var p)&&(p.ValueKind==JsonValueKind.True||(p.ValueKind==JsonValueKind.String&&bool.TryParse(p.GetString(),out var value)&&value));
    ...
    private ExchangeOrder Map(OrderResponse o){var type=o.Type.ToUpperInvariant();var protection=type is "STOP_MARKET" or "TAKE_PROFIT_MARKET";return new(C(o.Symbol),o.OrderId.ToString(CultureInfo.InvariantCulture),o.ClientOrderId,NormalizeStandardOrderStatus(o.Status),o.ExecutedQuantity,o.AvgPrice,type,ParseSide(o.PositionSide),protection,o.Time){ProtectionCoverage=protection&&o.ClosePosition?ProtectionCoverageKind.PositionWide:ProtectionCoverageKind.Unknown};}
    private ExchangeOrder MapRaw(JsonElement e){var type=S(e,"type").ToUpperInvariant();var protection=type is "STOP_MARKET" or "TAKE_PROFIT_MARKET";return new(C(S(e,"symbol")),e.GetProperty("orderId").GetRawText().Trim('"'),S(e,"clientOrderId"),NormalizeStandardOrderStatus(S(e,"status")),D(e,"executedQty"),D(e,"avgPrice"),type,ParseSide(S(e,"positionSide")),protection,DateTime.UtcNow){ProtectionCoverage=protection&&Bool(e,"closePosition")?ProtectionCoverageKind.PositionWide:ProtectionCoverageKind.Unknown};}
    private ExchangeOrder MapAlgo(JsonElement e){var type=S(e,"orderType").ToUpperInvariant();var updated=e.TryGetProperty("updateTime",out var u)&&u.TryGetInt64(out var ms)?DateTimeOffset.FromUnixTimeMilliseconds(ms).UtcDateTime:DateTime.UtcNow;return new(C(S(e,"symbol")),e.GetProperty("algoId").GetRawText().Trim('"'),S(e,"clientAlgoId"),S(e,"algoStatus"),D(e,"actualQty"),D(e,"actualPrice"),type,ParseSide(S(e,"positionSide")),true,updated){ProtectionCoverage=Bool(e,"closePosition")?ProtectionCoverageKind.PositionWide:ProtectionCoverageKind.Unknown};}
