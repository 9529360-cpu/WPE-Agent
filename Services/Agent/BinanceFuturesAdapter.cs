...
public sealed class BinanceFuturesAdapter : IExchangeProvider,IMarketDataProvider,IBrokerProvider,IProviderMarketCatalog,IProviderEnvironmentGuard,IRecentOrderProvider,IExchangeOrderFeeEvidenceReader,ICryptoInstrumentFundamentalReader
{
    ...
    public async Task SetMarginModeAsync(string symbol,bool isolated,CancellationToken ct) { try { _=await _api.PostSignedRawAsync("/fapi/v1/marginType",new Dictionary<string,string?>{{"symbol",N(symbol)},{"marginType",isolated?"ISOLATED":"CROSSED"}},ct); } catch(HttpRequestException ex) when(ex.Message.Contains("400")){} }
