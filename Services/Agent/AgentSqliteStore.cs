...
public sealed record PostTradeReviewV1(string Schema,string ClientOrderId,string CycleId,string Symbol,string Side,decimal EntryPrice,decimal ExitPrice,decimal Quantity,decimal Fees,string FeeBasis,decimal FeeRate,decimal EntrySlippageAmount,decimal ExitSlippageAmount,decimal TotalSlippageAmount,string SlippageBasis,decimal NetPnl,decimal ReturnPct,string Outcome,DateTimeOffset ClosedAtUtc,string? StrategyId,string StrategyVersion,string AttributionBasis);
...
public sealed class AgentSqliteStore
{
    ...
    internal AgentSqliteStore(string? path,Func<DateTimeOffset>? utcNow) { path??=AppDataPaths.File("agent.db");Directory.CreateDirectory(Path.GetDirectoryName(path)!);_cs=$"Data Source={path}";_utcNow=utcNow??(()=>DateTimeOffset.UtcNow);Initialize();EnsureMacroReleaseColumns();EnsureExecutionQualityColumns(); }
    ...
    private void EnsureExecutionQualityColumns(){using var c=new SqliteConnection(_cs);c.Open();EnsureColumn(c,"execution_events","expected_price","TEXT NOT NULL DEFAULT '0'");EnsureColumn(c,"trade_outcomes","entry_slippage_amount","TEXT NOT NULL DEFAULT '0'");EnsureColumn(c,"trade_outcomes","exit_slippage_amount","TEXT NOT NULL DEFAULT '0'");EnsureColumn(c,"trade_outcomes","total_slippage_amount","TEXT NOT NULL DEFAULT '0'");EnsureColumn(c,"trade_outcomes","slippage_basis","TEXT NOT NULL DEFAULT 'unavailable'");EnsureColumn(c,"trade_outcomes","close_expected_price","TEXT NOT NULL DEFAULT '0'");}
    ...
    CREATE TABLE IF NOT EXISTS execution_events(id INTEGER PRIMARY KEY AUTOINCREMENT,cycle_id TEXT,client_order_id TEXT UNIQUE,symbol TEXT,side TEXT,action TEXT,reduce_only INTEGER,quantity TEXT,avg_price TEXT,expected_price TEXT NOT NULL DEFAULT '0',status TEXT,occurred_at TEXT);
    ...
    CREATE TABLE IF NOT EXISTS trade_outcomes(id INTEGER PRIMARY KEY AUTOINCREMENT,client_order_id TEXT,cycle_id TEXT,symbol TEXT,side TEXT,entry_price TEXT,exit_price TEXT,quantity TEXT,gross_pnl TEXT,fees TEXT,fee_basis TEXT NOT NULL DEFAULT 'estimated-static-rate',fee_rate TEXT NOT NULL DEFAULT '0.0004',entry_slippage_amount TEXT NOT NULL DEFAULT '0',exit_slippage_amount TEXT NOT NULL DEFAULT '0',total_slippage_amount TEXT NOT NULL DEFAULT '0',slippage_basis TEXT NOT NULL DEFAULT 'unavailable',close_expected_price TEXT NOT NULL DEFAULT '0',net_pnl TEXT,return_pct TEXT,closed_at TEXT,strategy_id TEXT,strategy_version TEXT,attribution_basis TEXT NOT NULL DEFAULT 'legacy-version-only');
    ...
    {
        var rows=new List<PostTradeReviewV1>();await using var c=new SqliteConnection(_cs);await c.OpenAsync(ct);await using var q=c.CreateCommand();q.CommandText="SELECT client_order_id,cycle_id,symbol,side,entry_price,exit_price,quantity,fees,fee_basis,fee_rate,entry_slippage_amount,exit_slippage_amount,total_slippage_amount,slippage_basis,net_pnl,return_pct,closed_at,strategy_id,strategy_version,attribution_basis FROM trade_outcomes WHERE client_order_id IS NOT NULL ORDER BY closed_at DESC,id DESC LIMIT $limit";q.Parameters.AddWithValue("$limit",Math.Clamp(limit,1,100));await using var r=await q.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct)){var net=Decimal(r.GetString(14));rows.Add(new("wpe.post-trade-review/1.3",r.GetString(0),r.GetString(1),r.GetString(2),r.GetString(3),Decimal(r.GetString(4)),Decimal(r.GetString(5)),Decimal(r.GetString(6)),Decimal(r.GetString(7)),r.GetString(8),Decimal(r.GetString(9)),Decimal(r.GetString(10)),Decimal(r.GetString(11)),Decimal(r.GetString(12)),r.GetString(13),net,Decimal(r.GetString(15)),net>0?"win":net<0?"loss":"flat",DateTimeOffset.Parse(r.GetString(16),CultureInfo.InvariantCulture,DateTimeStyles.RoundtripKind),r.IsDBNull(17)?null:r.GetString(17),r.GetString(18),r.GetString(19)));}return rows;
    }
    ...
    public async Task UpsertStrategyAsync(StrategyProfile profile,CancellationToken ct)=>await Exec("INSERT OR REPLACE INTO strategy_registry(id,version,symbol,family,lifecycle,parameters_json,built_in,created_at,state_changed_at,quality_score,expectancy,max_drawdown,sharpe,validation_trades,shadow_observations,failure_streak,last_reason) VALUES($i,$v,$s,$f,$l,$p,$b,$c,$u,$q,$e,$d,$h,$t,$o,$x,$r)",ct,("$i",profile.Id),("$v",profile.Version),("$s",profile.Symbol),("$f",profile.Family.ToString()),("$l",profile.Lifecycle.ToString()),("$p",JsonSerializer.Serialize(profile.Parameters)),("$b",profile.BuiltIn?1:0),("$c",profile.CreatedAtUtc.ToString("O")),("$u",profile.StateChangedAtUtc?.ToString("O")), ("$q",profile.QualityScore),("$e",profile.Expectancy),("$d",profile.MaxDrawdown),("$h",profile.Sharpe),("$t",profile.ValidationTrades),("$o",profile.ShadowObservations),("$x",profile.FailureStreak),("$r",profile.LastReason));
