...
public sealed partial class AgentSqliteStore
{
    ...
    public async Task RecordExecutionAsync(string cycle,ExecutionIntent intent,ExchangeOrder order,string strategyVersion,CancellationToken ct)
    {
        ...
        if(!inserted){await EnsureIdenticalTradeOutcomeAsync(c,intent.ClientOrderId,cycle,intent.Symbol,intent.Side.ToString(),entry,price,quantity,fees,slippage,intent.ExpectedPrice,funding,net,ret,attribution.StrategyVersion,ct);return;}
