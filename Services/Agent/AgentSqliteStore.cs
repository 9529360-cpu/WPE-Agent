...
public sealed record PostTradeReviewV1(string Schema,string ClientOrderId,string CycleId,string Symbol,string Side,decimal EntryPrice,decimal ExitPrice,decimal Quantity,decimal Fees,decimal NetPnl,decimal ReturnPct,string Outcome,DateTimeOffset ClosedAtUtc,string StrategyVersion);
...
public sealed class AgentSqliteStore
{
    ...
    CREATE TABLE IF NOT EXISTS trade_outcomes(id INTEGER PRIMARY KEY AUTOINCREMENT,client_order_id TEXT,cycle_id TEXT,symbol TEXT,side TEXT,entry_price TEXT,exit_price TEXT,quantity TEXT,gross_pnl TEXT,fees TEXT,net_pnl TEXT,return_pct TEXT,closed_at TEXT,strategy_version TEXT);
    ...
    """;q.ExecuteNonQuery();EnsureColumn(c,"trading_review_queue_events","reason","TEXT NOT NULL DEFAULT ''");EnsureColumn(c,"trading_approval_requests","artifact_hash","TEXT");EnsureColumn(c,"trading_approval_receipts","artifact_hash","TEXT");EnsureColumn(c,"trade_outcomes","client_order_id","TEXT");using var outcomeIndex=c.CreateCommand();outcomeIndex.CommandText="CREATE UNIQUE INDEX IF NOT EXISTS ux_trade_outcomes_client_order ON trade_outcomes(client_order_id) WHERE client_order_id IS NOT NULL";outcomeIndex.ExecuteNonQuery();}
    ...
    public async Task RecordExecutionAsync(string cycle,ExecutionIntent intent,ExchangeOrder order,string strategyVersion,CancellationToken ct)
    {
        ...
        if(intent.ReduceOnly&&string.Equals(order.Status,"FILLED",StringComparison.OrdinalIgnoreCase)&&await AcceptIdenticalPostTradeReplayAsync(intent.ClientOrderId,cycle,intent.Symbol,intent.Side.ToString(),price,quantity,strategyVersion,ct))return;
        ...
        var gross=(intent.Side==PositionSide.Long?price-entry.Value:entry.Value-price)*quantity;var fees=(entry.Value+price)*quantity*.0004m;var net=gross-fees;var ret=entry.Value*quantity>0?net/(entry.Value*quantity):0;
        var closedAt=_utcNow().ToUniversalTime();await using var insert=c.CreateCommand();insert.CommandText="INSERT OR IGNORE INTO trade_outcomes(client_order_id,cycle_id,symbol,side,entry_price,exit_price,quantity,gross_pnl,fees,net_pnl,return_pct,closed_at,strategy_version) VALUES($id,$c,$s,$side,$e,$x,$q,$g,$f,$n,$r,$t,$v); SELECT changes();";insert.Parameters.AddWithValue("$id",intent.ClientOrderId);insert.Parameters.AddWithValue("$c",cycle);insert.Parameters.AddWithValue("$s",intent.Symbol);insert.Parameters.AddWithValue("$side",intent.Side.ToString());insert.Parameters.AddWithValue("$e",entry.Value.ToString(CultureInfo.InvariantCulture));insert.Parameters.AddWithValue("$x",price.ToString(CultureInfo.InvariantCulture));insert.Parameters.AddWithValue("$q",quantity.ToString(CultureInfo.InvariantCulture));insert.Parameters.AddWithValue("$g",gross.ToString(CultureInfo.InvariantCulture));insert.Parameters.AddWithValue("$f",fees.ToString(CultureInfo.InvariantCulture));insert.Parameters.AddWithValue("$n",net.ToString(CultureInfo.InvariantCulture));insert.Parameters.AddWithValue("$r",ret.ToString(CultureInfo.InvariantCulture));insert.Parameters.AddWithValue("$t",closedAt.ToString("O"));insert.Parameters.AddWithValue("$v",strategyVersion);var inserted=Convert.ToInt32(await insert.ExecuteScalarAsync(ct),CultureInfo.InvariantCulture)==1;
        if(!inserted){await EnsureIdenticalTradeOutcomeAsync(c,intent.ClientOrderId,cycle,intent.Symbol,intent.Side.ToString(),entry.Value,price,quantity,fees,net,ret,strategyVersion,ct);return;}
        var outcome=net>0?"win":net<0?"loss":"flat";await SaveMemoryAsync(new("long-term",closedAt.UtcDateTime,intent.Symbol,null,strategyVersion,outcome,"post-trade",$"schema=wpe.post-trade-review/1.0; symbol={intent.Symbol}; side={intent.Side}; outcome={outcome}; netPnl={net.ToString(CultureInfo.InvariantCulture)}; returnPct={ret.ToString(CultureInfo.InvariantCulture)}; fees={fees.ToString(CultureInfo.InvariantCulture)}"),ct);
    }
    ...
    {
        var rows=new List<PostTradeReviewV1>();await using var c=new SqliteConnection(_cs);await c.OpenAsync(ct);await using var q=c.CreateCommand();q.CommandText="SELECT client_order_id,cycle_id,symbol,side,entry_price,exit_price,quantity,fees,net_pnl,return_pct,closed_at,strategy_version FROM trade_outcomes WHERE client_order_id IS NOT NULL ORDER BY closed_at DESC,id DESC LIMIT $limit";q.Parameters.AddWithValue("$limit",Math.Clamp(limit,1,100));await using var r=await q.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct)){var net=Decimal(r.GetString(8));rows.Add(new("wpe.post-trade-review/1.0",r.GetString(0),r.GetString(1),r.GetString(2),r.GetString(3),Decimal(r.GetString(4)),Decimal(r.GetString(5)),Decimal(r.GetString(6)),Decimal(r.GetString(7)),net,Decimal(r.GetString(9)),net>0?"win":net<0?"loss":"flat",DateTimeOffset.Parse(r.GetString(10),CultureInfo.InvariantCulture,DateTimeStyles.RoundtripKind),r.GetString(11)));}return rows;
    }
