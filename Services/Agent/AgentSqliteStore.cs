...
public sealed partial class AgentSqliteStore
{
    ...
    public async Task RecordSkillCallAsync(string skill,string status,long duration,string input,string output,string? error,CancellationToken ct,string? mode=null)
    {
        ...
        await using(var q=c.CreateCommand()){q.Transaction=(SqliteTransaction)tx;q.CommandText="INSERT INTO runtime_skill_calls(source_skill_call_id,occurred_at,skill,status,duration_ms,mode) VALUES($i,$t,$s,$st,$d,$m)";q.Parameters.AddWithValue("$i",sourceId);q.Parameters.AddWithValue("$t",occurred.ToString("O"));q.Parameters.AddWithValue("$s",SensitiveDataRedactor.ForLog(skill,120));q.Parameters.AddWithValue("$st",SensitiveDataRedactor.ForLog(status,40));q.Parameters.AddWithValue("$d",Math.Max(0,duration));q.Parameters.AddWithValue("$m",mode is null?DBNull.Value:SensitiveDataRedactor.ForLog(mode,40));await q.ExecuteNonQueryAsync(ct);}await tx.CommitAsync(ct);
    }
    public async Task<IReadOnlyList<PersistedRuntimeSkillCall>> GetRecentRuntimeSkillCallsAsync(int limit,CancellationToken ct)
    {
        var list=new List<PersistedRuntimeSkillCall>();await using var c=new SqliteConnection(_cs);await c.OpenAsync(ct);await using var q=c.CreateCommand();q.CommandText="SELECT 'runtime:'||id,occurred_at,skill,status,duration_ms,mode FROM runtime_skill_calls UNION ALL SELECT 'legacy:'||s.id,s.occurred_at,s.skill,s.status,s.duration_ms,NULL FROM skill_calls s WHERE NOT EXISTS(SELECT 1 FROM runtime_skill_calls r WHERE r.source_skill_call_id=s.id) ORDER BY occurred_at DESC LIMIT $l";q.Parameters.AddWithValue("$l",Math.Clamp(limit,1,500));await using var r=await q.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct))list.Add(new(r.GetString(0),DateTime.Parse(r.GetString(1),null,DateTimeStyles.RoundtripKind).ToUniversalTime(),r.GetString(2),r.GetString(3),Math.Max(0,r.GetInt64(4)),r.IsDBNull(5)?null:r.GetString(5)));return list;
    }
...
public sealed record PersistedRuntimeSkillCall(string Id,DateTime OccurredAtUtc,string Skill,string Status,long DurationMs,string? Mode);
