...
public sealed class AgentSqliteStore
{
    ...
    internal AgentSqliteStore(string? path,Func<DateTimeOffset>? utcNow) { path??=AppDataPaths.File("agent.db");Directory.CreateDirectory(Path.GetDirectoryName(path)!);_cs=$"Data Source={path}";_utcNow=utcNow??(()=>DateTimeOffset.UtcNow);Initialize();EnsureMacroReleaseColumns(); }
    ...
    public async Task RecordExecutionAsync(string cycle,ExecutionIntent intent,ExchangeOrder order,string strategyVersion,CancellationToken ct)
    {
        ...
        if(intent.ReduceOnly&&string.Equals(order.Status,"FILLED",StringComparison.OrdinalIgnoreCase)&&await AcceptIdenticalPostTradeReplayAsync(intent.ClientOrderId,cycle,intent.Symbol,intent.Side.ToString(),price,quantity,attribution.StrategyVersion,ct))return;
    ...
    {
        var rows=new List<PostTradeReviewV1>();await using var c=new SqliteConnection(_cs);await c.OpenAsync(ct);await using var q=c.CreateCommand();q.CommandText="SELECT client_order_id,cycle_id,symbol,side,entry_price,exit_price,quantity,fees,fee_basis,fee_rate,net_pnl,return_pct,closed_at,strategy_id,strategy_version,attribution_basis FROM trade_outcomes WHERE client_order_id IS NOT NULL ORDER BY closed_at DESC,id DESC LIMIT $limit";q.Parameters.AddWithValue("$limit",Math.Clamp(limit,1,100));await using var r=await q.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct)){var net=Decimal(r.GetString(10));rows.Add(new("wpe.post-trade-review/1.2",r.GetString(0),r.GetString(1),r.GetString(2),r.GetString(3),Decimal(r.GetString(4)),Decimal(r.GetString(5)),Decimal(r.GetString(6)),Decimal(r.GetString(7)),r.GetString(8),Decimal(r.GetString(9)),net,Decimal(r.GetString(11)),net>0?"win":net<0?"loss":"flat",DateTimeOffset.Parse(r.GetString(12),CultureInfo.InvariantCulture,DateTimeStyles.RoundtripKind),r.IsDBNull(13)?null:r.GetString(13),r.GetString(14),r.GetString(15)));}return rows;
    }
    ...
    private static async Task EnsureIdenticalTradeOutcomeAsync(SqliteConnection c,string clientOrderId,string cycle,string symbol,string side,decimal entry,decimal exit,decimal quantity,decimal fees,decimal net,decimal returnPct,string strategyVersion,CancellationToken ct)
    {
        await using var q=c.CreateCommand();q.CommandText="SELECT cycle_id,symbol,side,entry_price,exit_price,quantity,fees,net_pnl,return_pct,strategy_version FROM trade_outcomes WHERE client_order_id=$id";q.Parameters.AddWithValue("$id",clientOrderId);await using var r=await q.ExecuteReaderAsync(ct);if(!await r.ReadAsync(ct)||!string.Equals(r.GetString(0),cycle,StringComparison.Ordinal)||!string.Equals(r.GetString(1),symbol,StringComparison.Ordinal)||!string.Equals(r.GetString(2),side,StringComparison.Ordinal)||Decimal(r.GetString(3))!=entry||Decimal(r.GetString(4))!=exit||Decimal(r.GetString(5))!=quantity||Decimal(r.GetString(6))!=fees||Decimal(r.GetString(7))!=net||Decimal(r.GetString(8))!=returnPct||!string.Equals(r.GetString(9),strategyVersion,StringComparison.Ordinal))throw new InvalidOperationException("Post-trade outcome identity conflict.");
    }
    private async Task<bool> AcceptIdenticalPostTradeReplayAsync(string clientOrderId,string cycle,string symbol,string side,decimal exit,decimal quantity,string strategyVersion,CancellationToken ct)
    {
        await using var c=new SqliteConnection(_cs);await c.OpenAsync(ct);await using var q=c.CreateCommand();q.CommandText="SELECT cycle_id,symbol,side,exit_price,quantity,strategy_version FROM trade_outcomes WHERE client_order_id=$id";q.Parameters.AddWithValue("$id",clientOrderId);await using var r=await q.ExecuteReaderAsync(ct);if(!await r.ReadAsync(ct))return false;if(!string.Equals(r.GetString(0),cycle,StringComparison.Ordinal)||!string.Equals(r.GetString(1),symbol,StringComparison.Ordinal)||!string.Equals(r.GetString(2),side,StringComparison.Ordinal)||Decimal(r.GetString(3))!=exit||Decimal(r.GetString(4))!=quantity||!string.Equals(r.GetString(5),strategyVersion,StringComparison.Ordinal))throw new InvalidOperationException("Post-trade outcome identity conflict.");return true;
    }
