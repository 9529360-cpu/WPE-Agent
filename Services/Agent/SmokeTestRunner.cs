...
public static class SmokeTestRunner
{
    public static async Task<SmokeRunResult> RunAsync(CancellationToken ct=default)
    {
        ...
        try
        {
            ...
            await Step(report,"大脑结构化决策",async()=>{var result=await brain.DecideAsync(evidence!,new(brain.Name,false,null,Array.Empty<string>()),ct);if(result.Decision.Instrument is not("BTCUSDT" or "ETHUSDT"))throw new InvalidOperationException("大脑返回了无效品种");return $"action={result.Decision.Action}, instrument={result.Decision.Instrument}, tier={result.Decision.TargetTier}, confidence={result.Decision.Confidence:F2}";});
