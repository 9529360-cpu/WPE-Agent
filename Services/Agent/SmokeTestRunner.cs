...
public static class SmokeTestRunner
{
    ...
    public static async Task<SmokeRunResult> RunAsync(CancellationToken ct=default)
    {
        ...
        try
        {
            ...
            await Step(report,"Model-off deterministic readiness",()=>
            {
                ...
                var verdict=ModelOffSmokeSafetyVerdict.Evaluate(review.Accepted&&!string.IsNullOrWhiteSpace(review.Explanation),assessment.Fresh);
        ...
        finally
        {
            ...
            if(exchange is not null)await exchange.DisposeAsync();report.CompletedAtUtc=DateTime.UtcNow;await global::币安量化机器人.Services.SensitiveDataRedactor.WriteRedactedJsonAsync(reportPath,report,CancellationToken.None);
        }
    ...
    internal static SmokeMutationGateResult AssessMutationReadiness(SmokeMutationReadiness readiness,DateTimeOffset now)
    {
        ...
        if(readiness.MissingSources.Count>0)return new(false,"smoke.readiness.evidence-source-missing");
