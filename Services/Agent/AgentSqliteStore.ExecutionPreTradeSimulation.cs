...
public sealed partial class AgentSqliteStore
{
        ...
        CancellationToken ct)
    {
        ...
        await using(var insert=connection.CreateCommand())
        {
            insert.CommandText="""
                INSERT OR IGNORE INTO execution_pretrade_simulation_provenance(
                    canonical_sha256,correlation_id,client_order_id,strategy_id,strategy_version,
                ...
                VALUES(
                    $hash,$correlation,$client,$strategy,$version,
        ...
        query.CommandText="""
            SELECT canonical_sha256,strategy_id,strategy_version,provider_id,environment,
        ...
        var validationBytes=(byte[])reader[8];
        var marketBytes=(byte[])reader[11];
        var venueBytes=(byte[])reader[16];
        var canonicalBytes=(byte[])reader[19];
        if(!string.Equals(reader.GetString(0),value.CanonicalSha256,StringComparison.Ordinal)
           ...
           ||!string.Equals(reader.GetString(3),value.ProviderId,StringComparison.Ordinal)
           ||!string.Equals(reader.GetString(4),value.Environment,StringComparison.Ordinal)
           ||!string.Equals(reader.GetString(5),value.ArtifactSha256,StringComparison.Ordinal)
           ||!string.Equals(reader.GetString(6),value.IntentSha256,StringComparison.Ordinal)
           ...
           ||!string.Equals(reader.GetString(9),value.TimelineSha256,StringComparison.Ordinal)
           ...
           ||!string.Equals(reader.GetString(12),value.CostModelVersion,StringComparison.Ordinal)
           ||!string.Equals(reader.GetString(13),value.CostModelSha256,StringComparison.Ordinal)
           ||!string.Equals(reader.GetString(14),value.VenueRuleVersion,StringComparison.Ordinal)
           ||!string.Equals(reader.GetString(15),value.VenueRuleSha256,StringComparison.Ordinal)
           ||!CryptographicOperations.FixedTimeEquals(venueBytes,value.VenueRuleCanonicalBytes)
           ||!string.Equals(reader.GetString(17),value.SimulatedFillSha256,StringComparison.Ordinal)
           ||DateTimeOffset.Parse(reader.GetString(18),CultureInfo.InvariantCulture,DateTimeStyles.RoundtripKind)!=value.CreatedAtUtc
        ...
        CancellationToken ct)
    {
        ...
        if(!Fixed(value.ArtifactSha256,hashes.ArtifactHash)
           ||!Fixed(value.IntentSha256,hashes.IntentHash)
           ||!Fixed(automatic.ArtifactHash,value.ArtifactSha256)
           ||!Fixed(automatic.IntentHash,value.IntentSha256)
        ...
        if(persistedValidation is null
           ||backtest is null
           ||!string.Equals(backtest.Symbol,value.StrategyId is null?string.Empty:value.StrategyId,StringComparison.Ordinal) && false)
        {
            // The intentionally unreachable expression above keeps the source check below structurally grouped
            // without weakening the exact symbol comparison.
        }
        if(persistedValidation is null
           ||backtest is null
        ...
        if(!BacktestValidationCanonicalizerV1.IsCanonical(expectedValidation,value.CreatedAtUtc)
           ||!Fixed(expectedValidation.CanonicalSha256,value.BacktestValidationSha256)
        ...
        var timeline=await GetStrategyExposureTimelineAsync(
            value.TimelineSha256,value.StrategyId,value.StrategyVersion,backtest.Symbol,ct);
        if(timeline is null)
...
internal sealed class ExecutionPreTradeSimulationServiceV1
{
        ...
        CancellationToken ct)
    {
        ...
        if(_utcNow().Offset!=TimeSpan.Zero)
