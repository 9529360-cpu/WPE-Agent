...
public abstract class RestExchangeProviderBase:IExchangeProvider,IMarketDataProvider,IBrokerProvider
{
    ...
    {
        var canonical=Symbols.ToCanonical(canonicalSymbol);var c15=await GetCandlesAsync(canonical,"15m",240,ct);var c1=await GetCandlesAsync(canonical,"1h",120,ct);var c4=await GetCandlesAsync(canonical,"4h",90,ct);if(c15.Count<31||c1.Count<31||c4.Count<31)throw new InvalidOperationException($"{canonical} market history is incomplete");
        var price=c15[^1].Close;var deriv=await GetDerivativesAsync(canonical,ct);var rsi=Rsi(c15);var age=DateTime.UtcNow-c15[^1].OpenTime;var returns=c15.Skip(1).Select((x,i)=>c15[i].Close>0?Math.Log((double)(x.Close/c15[i].Close)):0).TakeLast(96).ToArray();var realized=returns.Length>1?Math.Sqrt(returns.Select(x=>(x-returns.Average())*(x-returns.Average())).Average())*Math.Sqrt(96):0;var recent=c15.TakeLast(8).Select(x=>x.QuoteVolume).DefaultIfEmpty().Average();var baseline=c15.TakeLast(96).Select(x=>x.QuoteVolume).DefaultIfEmpty().Average();var relative=baseline>0?(double)(recent/baseline):0;var anomalies=new List<string>();if(age>TimeSpan.FromMinutes(35))anomalies.Add("stale_candles");if(relative<=0)anomalies.Add("volume_unavailable");var quality=new MarketQualityEvidence{RealizedVolatility=realized,RelativeVolume=relative,LiquidityScore=Math.Clamp(relative/2,.25,1),SourceCount=3,QualityScore=anomalies.Count==0?88:62,Anomalies=anomalies};
        return new(canonical,price,c15.TakeLast(40).Min(x=>x.Low),c15.TakeLast(40).Max(x=>x.High),rsi,Trend(c15,10),Trend(c1,10),Trend(c4,10),deriv,DateTime.UtcNow){Candles=c15,Quality=quality};
    }
