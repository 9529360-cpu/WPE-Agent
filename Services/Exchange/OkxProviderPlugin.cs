...
public sealed class OkxExchangeProvider:RestExchangeProviderBase,WpeAgent.RuntimeContracts.IProviderMarketCatalog
{
    ...
    private async Task<ExchangeOrder> MapOrder(JsonElement x,bool algo,CancellationToken ct){var id=algo?Str(x,"algoId"):Str(x,"ordId");if(algo)_algoIds[id]=0;var side=Str(x,"posSide")=="short"?PositionSide.Short:PositionSide.Long;var type=Str(x,"ordType").ToUpperInvariant();var updated=OrderTimestamp(Str(x,"uTime") is {Length:>0} t?t:Str(x,"cTime"));var value=await ContractValue(Str(x,"instId"),ct);var size=algo?Math.Abs(Dec(x,"sz"))*value:0m;return new(Symbols.ToCanonical(Str(x,"instId")),id,algo?Str(x,"algoClOrdId"):Str(x,"clOrdId"),Status(Str(x,"state")),Dec(x,"accFillSz")*value,Dec(x,"avgPx"),type,side,algo,updated){ProtectionCoverage=algo&&size>0?ProtectionCoverageKind.FixedQuantity:ProtectionCoverageKind.Unknown,ProtectionQuantity=size};}
