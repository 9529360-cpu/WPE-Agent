...
public sealed class BinanceProviderPlugin:IExchangeProviderPlugin
{
    public static ExchangeProviderDescriptor ProviderDescriptor { get; }=new(
        "binance-futures","Binance Futures",ExchangeAssetClass.CryptoCex,true,true,
        [new("apiKey","API Key",ExchangeCredentialKind.ApiKey),new("secret","API Secret",ExchangeCredentialKind.Secret)],
        new HashSet<string>(["account","balance","positions","margin","candles","orderbook","trades","funding","open-interest","index-price","mark-price","place-order","cancel-order","query-order","protection-orders","realtime","health"]));
