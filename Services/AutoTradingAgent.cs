...
public static class AutoTradingAgent
{
    ...
    private static async Task Run(CancellationToken ct)
    {
        ...
        var roles=AgentRoleRuntimeRegistry.Shared;roles.Publish("market","monitoring","Public market and Testnet streams are being monitored.");roles.Publish("research","waiting","Waiting for the next market-driven research cycle.");roles.Publish("strategy","monitoring","Direct local candle structure is the trading decision authority; strategy ranking and scoring are disabled for execution.");roles.Publish("risk","monitoring","Risk limits and execution authority are being monitored.");roles.Publish("execution","waiting","Execution queue is ready; no approved order is pending.");roles.Publish("recovery","monitoring","Order state and reconciliation queue are being monitored.");roles.Publish("audit","monitoring","Append-only runtime and decision audit is active.");
        ...
        while(!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                Stage("Stage.Evidence","OBSERVATION",32);var evidence=await SkillAsync("EvidenceCollector",string.Join(',',settings.Symbols),token=>collector.CollectAsync(token),x=>$"completeness={x.Completeness} markets={x.Markets.Count} missing={x.MissingSources.Count}",ct);await Db.SaveNewsAsync(evidence.News,ct);ServiceLocator.SystemState.StrategyStatus="DIRECT";ServiceLocator.SystemState.StrategySummary=DirectMarketStructureDecisionSkill.DecisionContextKind;ServiceLocator.SystemState.StrategyCandidates=0;await Db.SetStateAsync("strategy-portfolio:last-plan","{\"mode\":\"disabled\",\"authority\":\"direct-market-structure\"}",ct);roles.Publish("strategy","monitoring","Direct local candle-structure decisions are active; strategy ranking and portfolio scoring are not execution authority.");await Db.StartCycleAsync(cycle,evidence,brain.Name,ct);await runtime.BeginCycleAsync(cycle,new{evidence.Completeness,Markets=evidence.Markets.Keys,brain=brain.Name,decisionPath=DirectMarketStructureDecisionSkill.DecisionContextKind},ct);workflowStarted=true;
                Stage("Stage.Research","OBSERVATION",36);await runtime.TransitionAsync(cycle,WorkflowNode.Research,new{evidence.Completeness,MarketCount=evidence.Markets.Count,ExecutionAuthority="direct-market-structure"},ct);var research=new Dictionary<string,ResearchValidationResult>(StringComparer.OrdinalIgnoreCase);var histories=new Dictionary<string,IReadOnlyList<CandleEvidence>>(StringComparer.OrdinalIgnoreCase);foreach(var market in evidence.Markets.Values)histories[market.Symbol]=await Db.LoadHistoricalCandlesAsync(market.Symbol,"1h",30000,ct);
                var positionTradingRules=await ReadPositionManagementRulesAsync(exchange,positions,ct);
                Stage("Stage.PositionManagement","RISK",39);await runtime.TransitionAsync(cycle,WorkflowNode.PositionManagement,new{Positions=positions.Count,Research=research.Count},ct);var management=await SkillAsync("PositionManagement",$"positions={positions.Count}",token=>positionManager.EvaluateAsync(positions,evidence.Markets,Db,token,executionLedger,null,positionTradingRules),x=>$"intents={x.Intents.Count} adjustments={x.ProtectionAdjustments.Count}",ct);var managementActivity=false;
                ...
                Stage("Stage.Aggregation","OBSERVATION",43);await runtime.TransitionAsync(cycle,WorkflowNode.Aggregation,new{ManagementIntents=management.Intents.Count,managementActivity,DecisionPath=DirectMarketStructureDecisionSkill.DecisionContextKind},ct);IReadOnlyList<MarketDecisionAssessment> assessments=Array.Empty<MarketDecisionAssessment>();UpdateEvidence(evidence,assessments,research);
                ...
                MarketDecisionAssessment? proposedAssessment=null;evidence.Markets.TryGetValue(proposed.Instrument,out var proposedMarket);proposed=await SkillAsync("DeterministicPlan",$"{proposed.Action} {proposed.Instrument}",_=>Task.FromResult(deterministic.Complete(proposed,proposedMarket,proposedAssessment,settings.Risk)),x=>$"entry={x.EntryPrice} stop={x.StopLossPrice} take={x.TakeProfitPrice} RR={x.RiskRewardRatio:F2}",ct);
                Stage("Stage.Review","CRITIC",65);await runtime.TransitionAsync(cycle,WorkflowNode.Critic,new{proposed.Action,proposed.Instrument,DecisionPath=proposed.DecisionContextKind},ct);var review=await SkillAsync("DecisionReviewer",$"{proposed.Action} {proposed.Instrument}",_=>Task.FromResult(governance.Review(proposed,assessments,evidence,settings.Decision,null)),x=>$"accepted={x.Accepted} blocks={x.BlockingReasons.Count}",ct);var decision=review.Decision;MarketDecisionAssessment? selected=null;ResearchValidationResult? selectedResearch=null;UpdateDecisionUi(decision,review,selected,selectedResearch);
                ...
                var modelOffCycle=await RunModelOffProductionShadowAsync(Db,cycle,DateTimeOffset.UtcNow,evidence,research,assessments,review,riskReview,
                    positionReconciliation,protectionReconciliation,externalPositionIsolation,ct,null);
    ...
    {
        var state=ServiceLocator.SystemState;state.LastDecision=decision.Action.ToString();state.LastReason=review.Explanation;state.DecisionDiagnostics=review.Explanation;state.BrainConfidence=0;state.PlannedEntry=decision.EntryPrice;state.PlannedStop=decision.StopLossPrice;state.PlannedTakeProfit=decision.TakeProfitPrice;state.RiskRewardRatio=decision.RiskRewardRatio;state.ReviewerStatus=review.Accepted?"APPROVED":"REJECTED";state.ResearchScore=0;
        ...
        var directDriven=DirectMarketStructureDecisionSkill.IsDirect(decision);
        if(directDriven||hypothesisDriven)
        {
            state.DecisionScore=0;
            state.ConflictRate=0;
            state.RiskLoad=directDriven?0:Math.Clamp(decision.RiskBudgetMultiplier,0,1)*100;
