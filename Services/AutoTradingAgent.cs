...
public static class AutoTradingAgent
{
    ...
    private static async Task Run(CancellationToken ct)
    {
        ...
        await using IExchangeAdapter exchange=new BinanceFuturesAdapter(ExchangeEnvironment.Testnet,key,secret);_activeExchange=exchange;await using var realtime=new RealTimeMarketHub(ExchangeEnvironment.Testnet,settings.Symbols,key,Db);await realtime.StartAsync(ct);
