    ...
    private static async Task Run(CancellationToken ct)
    {
        ...
        while(!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                Stage("Stage.Recovery","OBSERVATION",25);var recoverable=await Db.GetRecoverableIntentsAsync(ct);var pendingAssessment=executionGateway.AssessUnverifiedAutomaticMutation(AutomaticMutationPath.StartupRecovery,recoverable.Count);var pendingRecovery=new RecoveryResult(pendingAssessment.SafeToIncreaseRisk,[pendingAssessment.Code]);orders=await ReadOrdersAsync(exchange,null,ct);ServiceLocator.RuntimeTrading.Publish(positions,orders);var reconciliationNow=DateTimeOffset.UtcNow;var protectionReconciliation=ProtectionReconciliationServiceV1.Reconcile(positions,orders,reconciliationNow,reconciliationNow);await Db.SaveProtectionReconciliationAsync(protectionReconciliation,ct);var observedAt=account.Timestamp.Kind==DateTimeKind.Utc?new DateTimeOffset(account.Timestamp):new DateTimeOffset(DateTime.SpecifyKind(account.Timestamp,DateTimeKind.Utc));var executionLedgerSnapshot=await Db.GetExecutionPositionLedgerSnapshotAsync(ct);var executionLedger=executionLedgerSnapshot.Legs;var positionReconciliation=PositionReconciliationServiceV1.Reconcile(executionLedger,positions,observedAt,reconciliationNow);await Db.SavePositionReconciliationAsync(positionReconciliation,ct);var positionDriftPersisted=await Db.PersistExecutionPositionDriftEvidenceSafelyAsync(executionLedgerSnapshot,positionReconciliation,ct);if(positionDriftPersisted&&positionReconciliation.State==PositionReconciliationStateV1.Confirmed&&executionLedgerSnapshot.DriftIntegrityValid&&executionLedgerSnapshot.DriftTrackedExecutionCount>0){await new ExecutionRealityCalibrationServiceV1(Db).BuildAndPersistSafelyAsync(ct);await new ExecutionRealityStabilityServiceV1(Db).BuildAndPersistSafelyAsync(ct);await new ExecutionRealitySensitivityServiceV1(Db).BuildAndPersistSafelyAsync(ct);}var externalPositionIsolation=ExternalPositionIsolationServiceV1.Evaluate(executionLedger,positions,observedAt,reconciliationNow);await Db.SaveExternalPositionIsolationAsync(externalPositionIsolation,ct);var safeToIncreaseRisk=pendingRecovery.SafeToIncreaseRisk&&protectionReconciliation.AllowsRiskIncrease&&positionReconciliation.AllowsRiskIncrease&&externalPositionIsolation.AllowsRiskIncrease;var protectionMessage=protectionReconciliation.AllowsRiskIncrease?"protection.reconciliation-confirmed":string.Join(',',protectionReconciliation.ReasonCodes);var positionMessage=positionReconciliation.AllowsRiskIncrease?"position.reconciliation-confirmed":string.Join(',',positionReconciliation.ReasonCodes);var isolationMessage=externalPositionIsolation.AllowsRiskIncrease?"external-position.isolation-clear":string.Join(',',externalPositionIsolation.ReasonCodes);var safetyMessage=string.Join("；",pendingRecovery.Messages.Append(protectionMessage).Append(positionMessage).Append(isolationMessage));if(!safeToIncreaseRisk)await Db.SetStateAsync("authorization.automatic-maintenance",safetyMessage,ct);
                ...
                Stage("Stage.Research","OBSERVATION",36);await runtime.TransitionAsync(cycle,WorkflowNode.Research,new{evidence.Completeness,MarketCount=evidence.Markets.Count},ct);var research=new Dictionary<string,ResearchValidationResult>(StringComparer.OrdinalIgnoreCase);var histories=new Dictionary<string,IReadOnlyList<CandleEvidence>>(StringComparer.OrdinalIgnoreCase);foreach(var market in evidence.Markets.Values){var series=await Db.LoadHistoricalCandlesAsync(market.Symbol,"1h",30000,ct);histories[market.Symbol]=series;if(!localSignals.TryGetValue(market.Symbol,out var activeSignal))continue;var validation=await SkillAsync("StrategyResearch",$"{market.Symbol} strategy={activeSignal.StrategyId} version={activeSignal.StrategyVersion}",token=>researchAuthority.ReadAsync(activeSignal.StrategyId,activeSignal.StrategyVersion,market.Symbol,token),x=>x is null?"exact strategy research unavailable":$"promoted={x.Promoted} score={x.QualityScore:F2} trades={x.Trades} coverage={x.CoverageDays}d",ct);if(validation is null)continue;research[market.Symbol]=validation;await Db.SaveResearchAsync(validation,ct);}
                ...
                if(intents.Count>0&&tradingRule is not null)
                {
                    ...
                    {
                        var activeStrategy=strategyProfiles.SingleOrDefault(value=>value.Lifecycle==StrategyLifecycle.Active&&value.Symbol.Equals(decision.Instrument,StringComparison.Ordinal)&&string.Equals(value.Id,decision.StrategyId,StringComparison.Ordinal)&&string.Equals(value.Version,decision.StrategyVersion,StringComparison.Ordinal));
