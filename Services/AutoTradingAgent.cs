    ...
    private static async Task Run(CancellationToken ct)
    {
        ...
        while(!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                Stage("Stage.Recovery","OBSERVATION",25);var recoverable=await Db.GetRecoverableIntentsAsync(ct);var pendingAssessment=executionGateway.AssessUnverifiedAutomaticMutation(AutomaticMutationPath.StartupRecovery,recoverable.Count);var pendingRecovery=new RecoveryResult(pendingAssessment.SafeToIncreaseRisk,[pendingAssessment.Code]);orders=await ReadOrdersAsync(exchange,null,ct);ServiceLocator.RuntimeTrading.Publish(positions,orders);var reconciliationNow=DateTimeOffset.UtcNow;var protectionReconciliation=ProtectionReconciliationServiceV1.Reconcile(positions,orders,reconciliationNow,reconciliationNow);await Db.SaveProtectionReconciliationAsync(protectionReconciliation,ct);var observedAt=account.Timestamp.Kind==DateTimeKind.Utc?new DateTimeOffset(account.Timestamp):new DateTimeOffset(DateTime.SpecifyKind(account.Timestamp,DateTimeKind.Utc));var positionReconciliation=PositionReconciliationServiceV1.Reconcile(await Db.GetExecutionPositionLedgerAsync(ct),positions,observedAt,reconciliationNow);await Db.SavePositionReconciliationAsync(positionReconciliation,ct);var safeToIncreaseRisk=pendingRecovery.SafeToIncreaseRisk&&protectionReconciliation.AllowsRiskIncrease&&positionReconciliation.AllowsRiskIncrease;var protectionMessage=protectionReconciliation.AllowsRiskIncrease?"protection.reconciliation-confirmed":string.Join(',',protectionReconciliation.ReasonCodes);var positionMessage=positionReconciliation.AllowsRiskIncrease?"position.reconciliation-confirmed":string.Join(',',positionReconciliation.ReasonCodes);var safetyMessage=string.Join("；",pendingRecovery.Messages.Append(protectionMessage).Append(positionMessage));if(!safeToIncreaseRisk)await Db.SetStateAsync("authorization.automatic-maintenance",safetyMessage,ct);
