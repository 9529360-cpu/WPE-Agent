    ...
    private static async Task Run(CancellationToken ct)
    {
        ...
        while(!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                Stage("Stage.Recovery","OBSERVATION",25);var pendingRecovery=await executor.RecoverPendingAsync(ct);orders=await exchange.GetOpenOrdersAsync(null,ct);var protection=await executor.AuditAndRepairProtectionAsync(positions,orders,ct);var safeToIncreaseRisk=pendingRecovery.SafeToIncreaseRisk&&protection.SafeToIncreaseRisk;var safetyMessage=string.Join("；",pendingRecovery.Messages.Concat(protection.Messages));
                ...
                Stage("Stage.Evidence","OBSERVATION",32);var evidence=await collector.CollectAsync(ct);await Db.SaveNewsAsync(evidence.News,ct);await Db.StartCycleAsync(cycle,evidence,brain.Name,ct);
                Stage("Stage.Research","OBSERVATION",36);var research=new Dictionary<string,ResearchValidationResult>(StringComparer.OrdinalIgnoreCase);foreach(var market in evidence.Markets.Values){var validation=researchSkill.Evaluate(market);research[market.Symbol]=validation;await Db.SaveResearchAsync(validation,ct);}
                Stage("Stage.PositionManagement","RISK",39);var management=await positionManager.EvaluateAsync(positions,evidence.Markets,Db,ct);var managementActivity=false;
                if(management.Intents.Count>0||management.ProtectionAdjustments.Count>0)
                {
                    await ExecutionGate.WaitAsync(ct);try
                    {
                        foreach(var rawIntent in management.Intents){var rule=await exchange.GetRulesAsync(rawIntent.Symbol,ct);var intent=rawIntent with{Quantity=rule.RoundQuantity(rawIntent.Quantity)};if(intent.Quantity>=rule.MinQuantity&&intent.Quantity*intent.ExpectedPrice>=rule.MinNotional){await executor.ExecuteAsync(cycle,intent,Math.Min(settings.Risk.Leverage,rule.MaxLeverage),settings.Risk.Isolated,ct);managementActivity=true;}}
                        foreach(var adjustment in management.ProtectionAdjustments){await executor.ReplaceProtectionAsync(cycle,adjustment,ct);managementActivity=true;}
                ...
                Stage("Stage.Aggregation","OBSERVATION",43);var assessments=aggregator.Analyze(evidence,settings.Decision);UpdateEvidence(evidence,assessments,research);
                ...
                var proposedAssessment=assessments.FirstOrDefault(x=>x.Symbol.Equals(proposed.Instrument,StringComparison.OrdinalIgnoreCase));evidence.Markets.TryGetValue(proposed.Instrument,out var proposedMarket);proposed=deterministic.Complete(proposed,proposedMarket,proposedAssessment,settings.Risk);
                ...
                var history=await Db.GetRiskHistoryAsync(ct);var riskReview=independentRisk.Review(decision,evidence,selected,intents,settings.Risk,history,selectedResearch);
                ...
                {
                    if(_paused)result=L("Agent.PausedBeforeExecution");else{await ExecutionGate.WaitAsync(ct);try{if(_paused)result=L("Agent.PausedBeforeExecution");else{Stage("Stage.Execution","EXECUTION",91);result=await executor.ExecutePlanAsync(cycle,intents,Math.Min(settings.Risk.Leverage,tradingRule.MaxLeverage),settings.Risk.Isolated,ct);state.ExecutionApprovalStatus="EXECUTED";}}finally{ExecutionGate.Release();}}
                }
