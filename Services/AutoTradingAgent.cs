...
public static class AutoTradingAgent
{
    ...
    private static async Task Run(CancellationToken ct)
    {
        ...
        try{var sync=await SkillAsync("HistoricalData",string.Join(',',settings.Symbols)+" interval=1h",token=>historicalData.SyncAsync(settings.Symbols,"1h",3,token),x=>string.Join(" | ",x.Select(s=>$"{s.Symbol}:{s.Stored}/{s.CoverageDays}d")),ct);ServiceLocator.SystemState.HistoricalCoverageDays=sync.Count==0?0:sync.Min(x=>x.CoverageDays);}
        catch(OperationCanceledException)when(ct.IsCancellationRequested){throw;}
        catch(Exception ex){await Db.RecordErrorAsync("HistoricalData1h",ex,CancellationToken.None);ServiceLocator.SystemState.HistoricalCoverageDays=0;ServiceLocator.SystemState.LastMessage=L("Agent.CycleDegraded",ex.Message);}
        try{var sync=await SkillAsync("HistoricalData",string.Join(',',settings.Symbols)+" interval=15m",token=>historicalData.SyncAsync(settings.Symbols,"15m",2,token),x=>string.Join(" | ",x.Select(s=>$"{s.Symbol}:{s.Stored}/{s.CoverageDays}d")),ct);if(sync.Count>0&&ServiceLocator.SystemState.HistoricalCoverageDays>0)ServiceLocator.SystemState.HistoricalCoverageDays=Math.Min(ServiceLocator.SystemState.HistoricalCoverageDays,sync.Min(x=>x.CoverageDays));}
        catch(OperationCanceledException)when(ct.IsCancellationRequested){throw;}
        catch(Exception ex){await Db.RecordErrorAsync("HistoricalData15m",ex,CancellationToken.None);ServiceLocator.SystemState.LastMessage=L("Agent.CycleDegraded",ex.Message);}
        try
        {
        ...
        while(!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                Stage("Stage.Evidence","OBSERVATION",32);var evidence=await SkillAsync("EvidenceCollector",string.Join(',',settings.Symbols),token=>collector.CollectAsync(token),x=>$"completeness={x.Completeness} markets={x.Markets.Count} missing={x.MissingSources.Count}",ct);await Db.SaveNewsAsync(evidence.News,ct);roles.Publish("strategy","running","Evaluating active signals and shadow candidates against fresh market evidence.");var strategyProfiles=await Db.GetStrategiesAsync(ct);var localSignals=strategyProfiles.Where(x=>x.Lifecycle==StrategyLifecycle.Active&&x.Family!=StrategyFamily.NumericalStructure).Select(x=>new{Profile=x,Signal=strategyResearch.GetSignal(x,evidence.Markets.GetValueOrDefault(x.Symbol)??new MarketEvidence(x.Symbol,0,0,0,50,0,0,0,new(0,0,1,1,1,1,0),DateTime.UtcNow),evidence.News)}).Where(x=>x.Signal.Direction!=0).GroupBy(x=>x.Profile.Symbol,StringComparer.OrdinalIgnoreCase).ToDictionary(x=>x.Key,x=>x.OrderByDescending(y=>y.Profile.QualityScore).First().Signal,StringComparer.OrdinalIgnoreCase);await strategyResearch.ObserveAsync(evidence,ct);var strategySnapshot=await Db.GetStrategySnapshotAsync(ct);ServiceLocator.SystemState.StrategyStatus=strategySnapshot.Status;ServiceLocator.SystemState.StrategySummary=strategySnapshot.ActiveStrategy;ServiceLocator.SystemState.StrategyCandidates=strategySnapshot.Candidates;roles.Publish("strategy","monitoring",$"Active and shadow strategies were evaluated; {strategySnapshot.Candidates} candidates remain under lifecycle monitoring.");await Db.StartCycleAsync(cycle,evidence,brain.Name,ct);await runtime.BeginCycleAsync(cycle,new{evidence.Completeness,Markets=evidence.Markets.Keys,brain=brain.Name,strategies=strategySnapshot.ActiveStrategy},ct);workflowStarted=true;
                Stage("Stage.Research","OBSERVATION",36);await runtime.TransitionAsync(cycle,WorkflowNode.Research,new{evidence.Completeness,MarketCount=evidence.Markets.Count},ct);var research=new Dictionary<string,ResearchValidationResult>(StringComparer.OrdinalIgnoreCase);var numericalResearch=new Dictionary<string,ResearchValidationResult>(StringComparer.OrdinalIgnoreCase);var histories=new Dictionary<string,IReadOnlyList<CandleEvidence>>(StringComparer.OrdinalIgnoreCase);foreach(var market in evidence.Markets.Values){var series=await Db.LoadHistoricalCandlesAsync(market.Symbol,"1h",30000,ct);histories[market.Symbol]=series;var validation=await SkillAsync("StrategyResearch",$"{market.Symbol} interval=1h candles={series.Count}",_=>Task.FromResult(longResearch.Evaluate(market.Symbol,series,settings.Risk)),x=>$"promoted={x.Promoted} score={x.QualityScore:F2} trades={x.Trades} coverage={x.CoverageDays}d",ct);research[market.Symbol]=validation;await Db.SaveResearchAsync(validation,ct);var numericalSeries=await Db.LoadHistoricalCandlesAsync(market.Symbol,"15m",100000,ct);var numericalValidation=await SkillAsync("NumericalStrategyResearch",$"{market.Symbol} interval=15m candles={numericalSeries.Count}",_=>Task.FromResult(numericalResearchSkill.Evaluate(market.Symbol,numericalSeries,settings.Risk)),x=>$"promoted={x.Promoted} score={x.QualityScore:F2} trades={x.Trades} coverage={x.CoverageDays}d",ct);numericalResearch[market.Symbol]=numericalValidation;await Db.SaveResearchAsync(numericalValidation,ct);}await strategyResearch.SyncNumericalStrategiesAsync(evidence,numericalResearch,settings.Risk,ct);strategyProfiles=await Db.GetStrategiesAsync(ct);strategySnapshot=await Db.GetStrategySnapshotAsync(ct);ServiceLocator.SystemState.StrategyStatus=strategySnapshot.Status;ServiceLocator.SystemState.StrategySummary=strategySnapshot.ActiveStrategy;ServiceLocator.SystemState.StrategyCandidates=strategySnapshot.Candidates;
                ...
                if((NumericalStrategySkill.IsNumerical(proposed)||RemoteEvidenceDecisionPolicy.IsRemote(proposed))&&numericalResearch.TryGetValue(proposed.Instrument,out var matchedNumericalResearch))research[proposed.Instrument]=matchedNumericalResearch;
                ...
                if(intents.Count>0&&tradingRule is not null)
                {
                    ...
                    {
                        var activeStrategy=SelectExecutionStrategyProfile(strategyProfiles,decision);
...
private static void UpdatePortfolioRiskUi(PortfolioRiskAssessment risk,IRealtimeMarketFeed realtime,ResearchValidationResult? research){var state=ServiceLocator.SystemState;state.RealtimeStatus=realtime.Status;state.HistoricalCoverageDays=research?.CoverageDays??state.HistoricalCoverageDays;state.PortfolioVaR99=risk.VaR99*100;state.PortfolioCVaR99=risk.CVaR99*100;state.PortfolioConcentration=risk.LargestPositionShare*100;state.PortfolioCorrelation=risk.MaximumPairCorrelation;state.PortfolioRiskSummary=risk.Summary;}
    ...
    internal static StrategyProfile? SelectExecutionStrategyProfile(IReadOnlyList<StrategyProfile> profiles,DecisionPlan decision)
    {
        var active=profiles.Where(value=>value.Lifecycle==StrategyLifecycle.Active&&value.Symbol.Equals(decision.Instrument,StringComparison.OrdinalIgnoreCase)).ToArray();
        if(NumericalStrategySkill.IsNumerical(decision)||RemoteEvidenceDecisionPolicy.IsRemote(decision))
            return active.Where(value=>value.Family==StrategyFamily.NumericalStructure&&value.Version.Equals(decision.StrategyVersion,StringComparison.Ordinal))
                .OrderByDescending(value=>value.QualityScore).ThenByDescending(value=>value.Expectancy).FirstOrDefault();
        return active.Where(value=>value.Family!=StrategyFamily.NumericalStructure)
            .OrderByDescending(value=>value.Version.Equals(decision.StrategyVersion,StringComparison.Ordinal))
            .ThenByDescending(value=>value.QualityScore).ThenByDescending(value=>value.Expectancy).FirstOrDefault();
    }

    private static bool ShouldUseRemotePlanner(EvidencePack evidence,IReadOnlyList<ManagedPosition> positions,int consecutiveHolds,DecisionPolicy policy)
    {
        if(!RemoteEvidenceDecisionPolicy.HasHardEvidence(evidence,policy))return false;
        if(positions.Count>0)return true;
        if(consecutiveHolds>=3)return true;
        return evidence.Markets.Count>0;
    }

    private static string? SelectPlannerMemorySymbol(EvidencePack evidence,IReadOnlyList<MarketDecisionAssessment> assessments)=>
        evidence.Markets.Values
            .Where(MarketEvidenceProvenanceCanonicalizerV1.IsCanonical)
            ...
            .Select(x=>x.Symbol)
            .FirstOrDefault()
        ??assessments.Where(x=>x.Fresh).OrderBy(x=>x.Symbol,StringComparer.OrdinalIgnoreCase).Select(x=>x.Symbol).FirstOrDefault();
