...
public static class AutoTradingAgent
{
    ...
    private static async Task Run(CancellationToken ct)
    {
        ...
        await using IExchangeAdapter exchange=new BinanceFuturesAdapter(ExchangeEnvironment.Testnet,key,secret,settings.Testnet);_activeExchange=exchange;await using var realtime=new RealTimeMarketHub(ExchangeEnvironment.Testnet,settings.Symbols,key,Db);await realtime.StartAsync(ct);
        ...
        ServiceLocator.SystemState.Mode=TradingMode.Testnet;ServiceLocator.SystemState.BrainName=brain.Name;var runtimeState=ServiceLocator.SystemState;runtimeState.ExchangeConnected=true;runtimeState.ApiTradePermission=settings.Testnet.TradePermission;Set(AgentStatus.Running,L("Agent.Started",ExchangeEnvironment.Testnet,brain.Name));var startupHealth=await brain.HealthCheckAsync(ct);runtimeState.BrainConnected=startupHealth.Healthy;if(!startupHealth.Healthy)throw new InvalidOperationException(L("Agent.BrainFailure",startupHealth.Message));Stage("Stage.HistoricalSync","OBSERVATION",12);
        try{var sync=await SkillAsync("HistoricalData",string.Join(',',settings.Symbols),()=>historicalData.SyncAsync(settings.Symbols,"1h",3,ct),x=>string.Join(" | ",x.Select(s=>$"{s.Symbol}:{s.Stored}/{s.CoverageDays}d")),ct);ServiceLocator.SystemState.HistoricalCoverageDays=sync.Count==0?0:sync.Min(x=>x.CoverageDays);}
        ...
        while(!ct.IsCancellationRequested)
        {
            ...
            catch(Exception ex){var state=ServiceLocator.SystemState;state.ExchangeConnected=false;state.ApiTradePermission=false;await Db.FailCycleAsync(cycle,state.SkillStage,ex,ct);await Db.RecordErrorAsync(state.SkillStage,ex,ct);state.LastError=ex.Message;Set(AgentStatus.Degraded,L("Agent.CycleDegraded",ex.Message));}
