...
public static class AutoTradingAgent
{
    ...
    public static void StartDefault(){if(IsRunning)return;var settings=SettingsStore.Load();if(string.IsNullOrWhiteSpace(settings.ActiveUser)||!settings.SetupCompleted)throw new InvalidOperationException(L("Access.StartBlocked"));var profile=SettingsStore.GetActiveExchange(settings);if(!profile.IsTestnet)throw new InvalidOperationException(L("Agent.MainnetConfirmationRequired"));if(settings.LastAccessCheckAtUtc is null||DateTime.UtcNow-settings.LastAccessCheckAtUtc>TimeSpan.FromMinutes(30))throw new InvalidOperationException(L("Access.StartBlocked"));_paused=false;_cts=new();_run=Task.Run(async()=>{try{await Run(_cts.Token);}catch(OperationCanceledException)when(_cts?.IsCancellationRequested==true){}catch(Exception ex){var state=ServiceLocator.SystemState;state.ExchangeConnected=false;state.BrainConnected=false;state.ApiTradePermission=false;await Db.RecordErrorAsync("AGENT_LOOP",ex,CancellationToken.None);state.LastError=ex.Message;Set(AgentStatus.Degraded,L("Agent.CycleDegraded",ex.Message));}finally{_activeExchange=null;}});}
    ...
    private static async Task Run(CancellationToken ct)
    {
        ...
        var exchangeProfile=SettingsStore.GetActiveExchange(settings);if(!exchangeProfile.IsTestnet)throw new InvalidOperationException(L("Agent.MainnetConfirmationRequired"));var credentials=SettingsStore.GetExchangeCredentials(exchangeProfile);if(credentials.Values.Any(string.IsNullOrWhiteSpace))throw new InvalidOperationException(L("Agent.ApiMissing"));
        ...
        await using IExchangeProvider exchange=Providers.Create(exchangeProfile,credentials);_activeExchange=exchange;await using var realtime=exchange.CreateRealtimeFeed(settings.Symbols,Db)??new PollingRealtimeFeed();await realtime.StartAsync(ct);
        var newsResearch=new NewsResearchService();var collector=new EvidenceCollector(exchange,settings.Symbols,realtime,newsResearch);var executor=new ReliableOrderExecutor(exchange,Db,settings.Risk);var planner=new RiskAndPositionPlanner();var aggregator=new SignalAggregationSkill();var governance=new DecisionGovernanceSkill();var deterministic=new DeterministicPlanSkill();var independentRisk=new IndependentRiskManagerSkill();var longResearch=new LongHorizonResearchSkill();var portfolioRiskSkill=new PortfolioRiskSkill();var historicalData=new HistoricalDataService(exchange,Db);var positionManager=new PositionManagementSkill();
        await runtime.RecoverInterruptedAsync(async token=>{var recovery=await executor.RecoverPendingAsync(token);return $"safe={recovery.SafeToIncreaseRisk}; messages={string.Join(';',recovery.Messages)}";},ct);
        ...
        ServiceLocator.SystemState.Mode=TradingMode.Testnet;ServiceLocator.SystemState.BrainName=brain.Name;var runtimeState=ServiceLocator.SystemState;runtimeState.ExchangeConnected=true;runtimeState.ApiTradePermission=exchangeProfile.TradePermission;Set(AgentStatus.Running,L("Agent.Started",exchangeProfile.DisplayName,brain.Name));var startupHealth=await brain.HealthCheckAsync(ct);runtimeState.BrainConnected=startupHealth.Healthy;if(!startupHealth.Healthy)throw new InvalidOperationException(L("Agent.BrainFailure",startupHealth.Message));Stage("Stage.HistoricalSync","OBSERVATION",12);
        ...
        while(!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                Stage("Stage.Recovery","OBSERVATION",25);var pendingRecovery=await SkillAsync("ProtectionRecovery","pending intents",token=>executor.RecoverPendingAsync(token),x=>$"safe={x.SafeToIncreaseRisk} messages={x.Messages.Count}",ct);orders=await exchange.GetOpenOrdersAsync(null,ct);var protection=await SkillAsync("ProtectionAudit",$"positions={positions.Count} orders={orders.Count}",token=>executor.AuditAndRepairProtectionAsync(positions,orders,token),x=>$"safe={x.SafeToIncreaseRisk} messages={x.Messages.Count}",ct);var safeToIncreaseRisk=pendingRecovery.SafeToIncreaseRisk&&protection.SafeToIncreaseRisk;var safetyMessage=string.Join("；",pendingRecovery.Messages.Concat(protection.Messages));
                ...
                Stage("Stage.Evidence","OBSERVATION",32);var evidence=await SkillAsync("EvidenceCollector",string.Join(',',settings.Symbols),token=>collector.CollectAsync(token),x=>$"completeness={x.Completeness} markets={x.Markets.Count} missing={x.MissingSources.Count}",ct);await Db.SaveNewsAsync(evidence.News,ct);await Db.StartCycleAsync(cycle,evidence,brain.Name,ct);await runtime.BeginCycleAsync(cycle,new{evidence.Completeness,Markets=evidence.Markets.Keys,brain=brain.Name},ct);workflowStarted=true;
                ...
                if(management.Intents.Count>0||management.ProtectionAdjustments.Count>0)
                {
                    ...
                    {
                        foreach(var rawIntent in management.Intents){var rule=await exchange.GetRulesAsync(rawIntent.Symbol,ct);var intent=rawIntent with{Quantity=rule.RoundQuantity(rawIntent.Quantity)};if(intent.Quantity>=rule.MinQuantity&&intent.Quantity*intent.ExpectedPrice>=rule.MinNotional){await SkillAsync("ReliableOrderExecutor",$"{intent.Symbol} {intent.Action} qty={intent.Quantity}",token=>executor.ExecuteAsync(cycle,intent,Math.Min(settings.Risk.Leverage,rule.MaxLeverage),settings.Risk.Isolated,token),x=>x,ct);managementActivity=true;}}
                        foreach(var adjustment in management.ProtectionAdjustments){await SkillAsync("ProtectionRecovery",$"{adjustment.Symbol} {adjustment.Side}",token=>executor.ReplaceProtectionAsync(cycle,adjustment,token),x=>x,ct);managementActivity=true;}
                ...
                Stage("Stage.Aggregation","OBSERVATION",43);await runtime.TransitionAsync(cycle,WorkflowNode.Aggregation,new{ManagementIntents=management.Intents.Count,managementActivity},ct);var assessments=await SkillAsync("SignalAggregation",$"markets={evidence.Markets.Count}",_=>Task.FromResult(aggregator.Analyze(evidence,settings.Decision)),x=>$"assessments={x.Count} ready={x.Count(a=>a.EntryReady)}",ct);UpdateEvidence(evidence,assessments,research);
                ...
                try{var activeSymbols=positions.Select(x=>x.Symbol).Distinct().ToArray();var outcomes=await Db.RecentOutcomesAsync(ct);var holdCount=await Db.ConsecutiveHoldCountAsync(ct);var brainResult=await SkillAsync("BrainPlanner",$"markets={evidence.Markets.Count} holds={holdCount}",token=>brain.DecideAsync(evidence,new(brain.Name,_dayHigh>0&&(_dayHigh-account.Equity)/_dayHigh>=settings.Risk.DailyDrawdownLimit,activeSymbols.Length==1?activeSymbols[0]:null,outcomes,assessments,holdCount),token),x=>$"{x.Decision.Action} {x.Decision.Instrument} confidence={x.Decision.Confidence:F2}",ct);proposed=brainResult.Decision;brainRequest=brainResult.Request;brainResponse=brainResult.Response;}
                ...
                {
                    if(_paused)result=L("Agent.PausedBeforeExecution");else{await ExecutionGate.WaitAsync(ct);try{if(_paused)result=L("Agent.PausedBeforeExecution");else{Stage("Stage.Execution","EXECUTION",91);await runtime.TransitionAsync(cycle,WorkflowNode.Execution,new{IntentCount=intents.Count,RiskApproved=riskReview.Approved},ct);result=await SkillAsync("ReliableOrderExecutor",$"intents={intents.Count}",token=>executor.ExecutePlanAsync(cycle,intents,Math.Min(settings.Risk.Leverage,tradingRule.MaxLeverage),settings.Risk.Isolated,token),x=>x,ct);state.ExecutionApprovalStatus="EXECUTED";}}finally{ExecutionGate.Release();}}
                }
            ...
            catch(Exception ex){var state=ServiceLocator.SystemState;state.ExchangeConnected=false;state.ApiTradePermission=false;if(workflowStarted)await runtime.FailCycleAsync(cycle,ex,CancellationToken.None);await Db.FailCycleAsync(cycle,state.SkillStage,ex,ct);await Db.RecordErrorAsync(state.SkillStage,ex,ct);state.LastError=ex.Message;Set(AgentStatus.Degraded,L("Agent.CycleDegraded",ex.Message));}
...
private static void UpdatePortfolioRiskUi(PortfolioRiskAssessment risk,IRealtimeMarketFeed realtime,ResearchValidationResult? research){var state=ServiceLocator.SystemState;state.RealtimeStatus=realtime.Status;state.HistoricalCoverageDays=research?.CoverageDays??state.HistoricalCoverageDays;state.PortfolioVaR99=risk.VaR99*100;state.PortfolioCVaR99=risk.CVaR99*100;state.PortfolioConcentration=risk.LargestPositionShare*100;state.PortfolioCorrelation=risk.MaximumPairCorrelation;state.PortfolioRiskSummary=risk.Summary;}
    ...
    {
        var descriptor=SkillGuard.Authorize(name,ServiceLocator.SystemState.Mode);using var timeout=CancellationTokenSource.CreateLinkedTokenSource(ct);timeout.CancelAfter(TimeSpan.FromSeconds(descriptor.TimeoutSeconds));var stopwatch=Stopwatch.StartNew();try{var value=await call(timeout.Token);await Db.RecordSkillCallAsync(name,"SUCCESS",stopwatch.ElapsedMilliseconds,input,Trim(summarize(value)),null,ct);return value;}catch(Exception ex){await Db.RecordSkillCallAsync(name,"FAILED",stopwatch.ElapsedMilliseconds,input,string.Empty,Trim(ex.Message),CancellationToken.None);throw;}
    }
