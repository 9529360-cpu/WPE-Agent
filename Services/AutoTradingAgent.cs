...
public static class AutoTradingAgent
{
    ...
    public static void StartDefault(){if(IsRunning)return;var settings=SettingsStore.Load();if(string.IsNullOrWhiteSpace(settings.ActiveUser)||!settings.SetupCompleted)throw new InvalidOperationException(L("Access.StartBlocked"));var profile=SettingsStore.GetActiveExchange(settings);if(!profile.IsTestnet)throw new InvalidOperationException(L("Agent.MainnetConfirmationRequired"));if(settings.LastAccessCheckAtUtc is null||DateTime.UtcNow-settings.LastAccessCheckAtUtc>TimeSpan.FromMinutes(30))throw new InvalidOperationException(L("Access.StartBlocked"));ServiceLocator.RuntimeTrading.MarkUnsupported("Waiting for the first provider position/order observation.");_paused=false;var runCts=new CancellationTokenSource();_cts=runCts;_run=Task.Run(async()=>{try{await Run(runCts.Token);}catch(OperationCanceledException)when(runCts.IsCancellationRequested){}catch(Exception ex){var state=ServiceLocator.SystemState;state.ExchangeConnected=false;state.BrainConnected=false;state.ApiTradePermission=false;await Db.RecordErrorAsync("AGENT_LOOP",ex,CancellationToken.None);await NotifyAgentDegradedAsync("agent-loop","AGENT_LOOP",ex);state.LastError=ex.Message;Set(AgentStatus.Degraded,L("Agent.CycleDegraded",ex.Message));}finally{_activeRuntimeSessionId=null;_activeCapabilityRefresh=null;_activeExecutionGateway=null;_activeExchange=null;}});}
    ...
    public static async Task StopAsync(){var runCts=_cts;if(runCts is null){ServiceLocator.RuntimeTrading.MarkUnsupported("Trading runtime is stopped.");return;}runCts.Cancel();try{if(_run is not null)await _run;}catch(OperationCanceledException){}runCts.Dispose();if(ReferenceEquals(_cts,runCts)){_cts=null;_run=null;}ServiceLocator.RuntimeTrading.MarkUnsupported("Trading runtime is stopped.");Set(AgentStatus.Stopped,L("Agent.Stopped"));}
    ...
    private static async Task Run(CancellationToken ct)
    {
        ...
        var automaticWorker=new AutomaticExecutionWorker(new AutomaticExecutionProcessor(Db,automaticValidator,automaticGateway));Task? automaticWorkerTask=null;
        ...
        while(!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                Stage("Stage.Recovery","OBSERVATION",25);var recoverable=await Db.GetRecoverableIntentsAsync(ct);var pendingAssessment=executionGateway.AssessUnverifiedAutomaticMutation(AutomaticMutationPath.StartupRecovery,recoverable.Count);var pendingRecovery=new RecoveryResult(pendingAssessment.SafeToIncreaseRisk,[pendingAssessment.Code]);orders=await ReadOrdersAsync(exchange,null,ct);ServiceLocator.RuntimeTrading.Publish(positions,orders);var protectionAssessment=executionGateway.AssessUnverifiedAutomaticMutation(AutomaticMutationPath.ProtectionAudit,positions.Count);var protection=new RecoveryResult(protectionAssessment.SafeToIncreaseRisk,[protectionAssessment.Code]);var observedAt=account.Timestamp.Kind==DateTimeKind.Utc?new DateTimeOffset(account.Timestamp):new DateTimeOffset(DateTime.SpecifyKind(account.Timestamp,DateTimeKind.Utc));var positionReconciliation=PositionReconciliationServiceV1.Reconcile(await Db.GetExecutionPositionLedgerAsync(ct),positions,observedAt,DateTimeOffset.UtcNow);await Db.SavePositionReconciliationAsync(positionReconciliation,ct);var safeToIncreaseRisk=pendingRecovery.SafeToIncreaseRisk&&protection.SafeToIncreaseRisk&&positionReconciliation.AllowsRiskIncrease;var positionMessage=positionReconciliation.AllowsRiskIncrease?"position.reconciliation-confirmed":string.Join(',',positionReconciliation.ReasonCodes);var safetyMessage=string.Join("；",pendingRecovery.Messages.Concat(protection.Messages).Append(positionMessage));if(!safeToIncreaseRisk)await Db.SetStateAsync("authorization.automatic-maintenance",safetyMessage,ct);
                ...
                Stage("Stage.Evidence","OBSERVATION",32);var evidence=await SkillAsync("EvidenceCollector",string.Join(',',settings.Symbols),token=>collector.CollectAsync(token),x=>$"completeness={x.Completeness} markets={x.Markets.Count} missing={x.MissingSources.Count}",ct);await Db.SaveNewsAsync(evidence.News,ct);if(DateTime.UtcNow>=nextStrategyResearch){try{await strategyResearch.RunOnceAsync(settings.Symbols,settings.Risk,ct);nextStrategyResearch=DateTime.UtcNow.AddHours(1);}catch(Exception ex){await Db.RecordErrorAsync("StrategyResearch",ex,CancellationToken.None);}}var strategyProfiles=await Db.GetStrategiesAsync(ct);var localSignals=strategyProfiles.Where(x=>x.Lifecycle==StrategyLifecycle.Active).Select(x=>new{Profile=x,Signal=strategyResearch.GetSignal(x,evidence.Markets.GetValueOrDefault(x.Symbol)??new MarketEvidence(x.Symbol,0,0,0,50,0,0,0,new(0,0,1,1,1,1,0),DateTime.UtcNow),evidence.News)}).Where(x=>x.Signal.Direction!=0).GroupBy(x=>x.Profile.Symbol,StringComparer.OrdinalIgnoreCase).ToDictionary(x=>x.Key,x=>x.OrderByDescending(y=>y.Profile.QualityScore).First().Signal,StringComparer.OrdinalIgnoreCase);await strategyResearch.ObserveAsync(evidence,ct);var strategySnapshot=await Db.GetStrategySnapshotAsync(ct);ServiceLocator.SystemState.StrategyStatus=strategySnapshot.Status;ServiceLocator.SystemState.StrategySummary=strategySnapshot.ActiveStrategy;ServiceLocator.SystemState.StrategyCandidates=strategySnapshot.Candidates;await Db.StartCycleAsync(cycle,evidence,brain.Name,ct);await runtime.BeginCycleAsync(cycle,new{evidence.Completeness,Markets=evidence.Markets.Keys,brain=brain.Name,strategies=strategySnapshot.ActiveStrategy},ct);workflowStarted=true;
                ...
                if(intents.Count>0&&tradingRule is not null)
                {
                    ...
                    {
                        var activeStrategy=strategyProfiles.FirstOrDefault(value=>value.Lifecycle==StrategyLifecycle.Active&&value.Symbol.Equals(decision.Instrument,StringComparison.OrdinalIgnoreCase));
            ...
            catch(Exception ex){var state=ServiceLocator.SystemState;state.ExchangeConnected=false;state.ApiTradePermission=false;if(workflowStarted)await runtime.FailCycleAsync(cycle,ex,CancellationToken.None);await Db.FailCycleAsync(cycle,state.SkillStage,ex,ct);await Db.RecordErrorAsync(state.SkillStage,ex,ct);await NotifyAgentDegradedAsync(cycle,state.SkillStage,ex);state.LastError=ex.Message;Set(AgentStatus.Degraded,L("Agent.CycleDegraded",ex.Message));}
            ServiceLocator.SystemState.NextCycleAtUtc=DateTime.UtcNow.AddMinutes(15);ServiceLocator.SystemState.RealtimeStatus=realtime.Status;StateChanged?.Invoke();_ = await realtime.WaitForTriggerAsync(TimeSpan.FromMinutes(15),ct);
        }
        }
        finally
        {
...
private static void UpdatePortfolioRiskUi(PortfolioRiskAssessment risk,IRealtimeMarketFeed realtime,ResearchValidationResult? research){var state=ServiceLocator.SystemState;state.RealtimeStatus=realtime.Status;state.HistoricalCoverageDays=research?.CoverageDays??state.HistoricalCoverageDays;state.PortfolioVaR99=risk.VaR99*100;state.PortfolioCVaR99=risk.CVaR99*100;state.PortfolioConcentration=risk.LargestPositionShare*100;state.PortfolioCorrelation=risk.MaximumPairCorrelation;state.PortfolioRiskSummary=risk.Summary;}
    ...
    private static void ApplyRuntimeHealth(RuntimeHealth health){var state=ServiceLocator.SystemState;state.RuntimeRunId=health.RunId;state.RuntimeHeartbeatAtUtc=health.HeartbeatAtUtc;state.RuntimeRecoveryStatus=health.RecoveryStatus;state.RuntimeEventSequence=health.EventSequence;StateChanged?.Invoke();}
