...
export type RuntimeHistoricalReconciliation={kind:'position'|'protection'|'externalIsolation';reportId:string;schema:string;observedAtUtc:string;evaluatedAtUtc:string;state:string;allowsRiskIncrease:boolean;canonicalSha256:string}
...
function normalizeHistoricalReconciliation(input:unknown):RuntimeHistoricalReconciliation|null{if(!input||typeof input!=='object')return null;const x=input as Record<string,unknown>,kind=nonEmptyString(x.kind),reportId=nonEmptyString(x.reportId),schema=nonEmptyString(x.schema),observedAtUtc=nonEmptyString(x.observedAtUtc),evaluatedAtUtc=nonEmptyString(x.evaluatedAtUtc),state=nonEmptyString(x.state),canonicalSha256=nonEmptyString(x.canonicalSha256);if(!kind||!['position','protection','externalIsolation'].includes(kind)||!reportId||!schema||!observedAtUtc||!evaluatedAtUtc||Number.isNaN(Date.parse(observedAtUtc))||Number.isNaN(Date.parse(evaluatedAtUtc))||!state||typeof x.allowsRiskIncrease!=='boolean'||!canonicalSha256||!/^[A-Fa-f0-9]{64}$/.test(canonicalSha256))return null;return{kind:kind as RuntimeHistoricalReconciliation['kind'],reportId,schema,observedAtUtc,evaluatedAtUtc,state,allowsRiskIncrease:x.allowsRiskIncrease,canonicalSha256}}
