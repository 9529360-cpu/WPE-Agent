...
export type WpeRuntimeState = {
  ...
  historicalPostTradeReviews?:RuntimeHistoricalCollection<RuntimeHistoricalPostTradeReview>
  historicalReconciliations?:RuntimeHistoricalCollection<RuntimeHistoricalReconciliation>
}
...
export type RuntimeHistoricalEvidenceLink={stage:'market'|'research'|'strategy'|'risk';status:string;canonicalSha256:string;asOfUtc:string}
export type RuntimeHistoricalPostTradeReview={traceId:string;schema:string;symbol:string;side:string;entryPrice:number;exitPrice:number;quantity:number;fees:number;feeBasis:string;feeRate:number;entrySlippageAmount:number;exitSlippageAmount:number;totalSlippageAmount:number;slippageBasis:string;fundingAmount:number;fundingBasis:string;netPnl:number;returnPct:number;outcome:'win'|'loss'|'flat';closedAtUtc:string;strategyId:string|null;strategyVersion:string;attributionBasis:string;traceState:'available'|'legacy'|'ambiguous';riskDecision:'Approved'|'Blocked'|'Unavailable'|'Conflicting'|'Ambiguous';executionStatus:string;executionCode:string;executionAttempts:number|null;marketCollectedAtUtc:string|null;marketDataVersion:string|null;evidenceState:'available'|'partial'|'legacy'|'unavailable'|'invalid'|'ambiguous';evidenceChain:RuntimeHistoricalEvidenceLink[]}
export type RuntimeHistoricalReconciliation={kind:'position'|'protection'|'externalIsolation';traceId:string;schema:string;observedAtUtc:string;evaluatedAtUtc:string;state:string;allowsRiskIncrease:boolean;canonicalSha256:string}
export type RuntimeHistoricalPageKind='orders'|'equity'|'backtests'|'skillCalls'|'auditEvents'|'postTradeReviews'|'reconciliations'
export type RuntimeHistoricalPageResponse={requestId:string;kind:RuntimeHistoricalPageKind;runtimePatch:Partial<WpeRuntimeState>}
...
function normalizeHistoricalOrder(input:unknown):RuntimeHistoricalOrder|null{if(!input||typeof input!=='object')return null;const x=input as Record<string,unknown>,sequence=finiteNumber(x.sequence),occurredAtUtc=nonEmptyString(x.occurredAtUtc),symbol=nonEmptyString(x.symbol),side=nonEmptyString(x.side),action=nonEmptyString(x.action),quantity=finiteNumber(x.quantity),status=nonEmptyString(x.status);const correlationId=x.correlationId===null||x.correlationId===undefined?null:nonEmptyString(x.correlationId),clientOrderId=x.clientOrderId===null||x.clientOrderId===undefined?null:nonEmptyString(x.clientOrderId),averagePrice=x.averagePrice===null?null:finiteNumber(x.averagePrice);if(sequence===null||!Number.isInteger(sequence)||!occurredAtUtc||Number.isNaN(Date.parse(occurredAtUtc))||!symbol||!side||!action||typeof x.reduceOnly!=='boolean'||quantity===null||quantity<0||!status||(averagePrice!==null&&averagePrice<0)||(correlationId!==null&&!/^cycle#[A-Fa-f0-9]{12}$/.test(correlationId))||(clientOrderId!==null&&!/^order#[A-Fa-f0-9]{12}$/.test(clientOrderId)))return null;return{sequence,occurredAtUtc,correlationId,clientOrderId,symbol,side,action,reduceOnly:x.reduceOnly,quantity,averagePrice,status}}
...
function normalizeHistoricalAudit(input:unknown):RuntimeHistoricalAuditEvent|null{if(!input||typeof input!=='object')return null;const x=input as Record<string,unknown>,id=nonEmptyString(x.id),occurredAtUtc=nonEmptyString(x.occurredAtUtc),category=nonEmptyString(x.category),source=nonEmptyString(x.source),status=nonEmptyString(x.status),correlationId=x.correlationId===null||x.correlationId===undefined?null:nonEmptyString(x.correlationId);if(!id||!occurredAtUtc||Number.isNaN(Date.parse(occurredAtUtc))||!category||!source||!status||(correlationId!==null&&!/^correlation#[A-Fa-f0-9]{12}$/.test(correlationId)))return null;return{id,occurredAtUtc,category,source,correlationId,status}}
function normalizeHistoricalEvidenceLink(input:unknown):RuntimeHistoricalEvidenceLink|null{if(!input||typeof input!=='object')return null;const x=input as Record<string,unknown>;if('canonicalBytes'in x||'sourcesJson'in x||'outputId'in x||'cycleId'in x)return null;const stage=nonEmptyString(x.stage),status=nonEmptyString(x.status),canonicalSha256=nonEmptyString(x.canonicalSha256),asOfUtc=nonEmptyString(x.asOfUtc);if(!stage||!['market','research','strategy','risk'].includes(stage)||!status||!canonicalSha256||!/^[A-Fa-f0-9]{64}$/.test(canonicalSha256)||!asOfUtc||Number.isNaN(Date.parse(asOfUtc)))return null;return{stage:stage as RuntimeHistoricalEvidenceLink['stage'],status,canonicalSha256,asOfUtc}}
function normalizeHistoricalPostTradeReview(input:unknown):RuntimeHistoricalPostTradeReview|null{if(!input||typeof input!=='object')return null;const x=input as Record<string,unknown>;if('clientOrderId'in x||'cycleId'in x||'executionId'in x)return null;const traceId=nonEmptyString(x.traceId),schema=nonEmptyString(x.schema),symbol=nonEmptyString(x.symbol),side=nonEmptyString(x.side),feeBasis=nonEmptyString(x.feeBasis),slippageBasis=nonEmptyString(x.slippageBasis),fundingBasis=nonEmptyString(x.fundingBasis),outcome=nonEmptyString(x.outcome),closedAtUtc=nonEmptyString(x.closedAtUtc),strategyVersion=nonEmptyString(x.strategyVersion),attributionBasis=nonEmptyString(x.attributionBasis),traceState=nonEmptyString(x.traceState),riskDecision=nonEmptyString(x.riskDecision),executionStatus=nonEmptyString(x.executionStatus),executionCode=nonEmptyString(x.executionCode),strategyId=x.strategyId===null?null:nonEmptyString(x.strategyId),marketCollectedAtUtc=x.marketCollectedAtUtc===null?null:nonEmptyString(x.marketCollectedAtUtc),marketDataVersion=x.marketDataVersion===null?null:nonEmptyString(x.marketDataVersion),evidenceState=nonEmptyString(x.evidenceState);const entryPrice=finiteNumber(x.entryPrice),exitPrice=finiteNumber(x.exitPrice),quantity=finiteNumber(x.quantity),fees=finiteNumber(x.fees),feeRate=finiteNumber(x.feeRate),entrySlippageAmount=finiteNumber(x.entrySlippageAmount),exitSlippageAmount=finiteNumber(x.exitSlippageAmount),totalSlippageAmount=finiteNumber(x.totalSlippageAmount),fundingAmount=finiteNumber(x.fundingAmount),netPnl=finiteNumber(x.netPnl),returnPct=finiteNumber(x.returnPct),attempts=x.executionAttempts===null?null:finiteNumber(x.executionAttempts);if(!Array.isArray(x.evidenceChain))return null;const evidenceChain=x.evidenceChain.map(normalizeHistoricalEvidenceLink);if(evidenceChain.some(link=>link===null))return null;const chain=evidenceChain as RuntimeHistoricalEvidenceLink[],stages=chain.map(link=>link.stage);const uniqueStages=new Set(stages);const evidenceStateValid=!!evidenceState&&['available','partial','legacy','unavailable','invalid','ambiguous'].includes(evidenceState);const evidenceShapeValid=uniqueStages.size===stages.length&&((evidenceState==='available'&&chain.length===4&&['market','research','strategy','risk'].every(stage=>uniqueStages.has(stage as RuntimeHistoricalEvidenceLink['stage'])))||(evidenceState==='partial'&&chain.length>0&&chain.length<4)||(['legacy','unavailable','invalid','ambiguous'].includes(evidenceState??'')&&chain.length===0));if(!traceId||!/^trade#[A-Fa-f0-9]{12}$/.test(traceId)||schema!=='wpe.post-trade-review/1.4'||!symbol||!side||entryPrice===null||entryPrice<=0||exitPrice===null||exitPrice<=0||quantity===null||quantity<=0||fees===null||fees<0||!feeBasis||feeRate===null||feeRate<0||entrySlippageAmount===null||exitSlippageAmount===null||totalSlippageAmount===null||!slippageBasis||fundingAmount===null||!fundingBasis||netPnl===null||returnPct===null||!outcome||!['win','loss','flat'].includes(outcome)||!closedAtUtc||Number.isNaN(Date.parse(closedAtUtc))||strategyId===undefined||!strategyVersion||!attributionBasis||!traceState||!['available','legacy','ambiguous'].includes(traceState)||!riskDecision||!['Approved','Blocked','Unavailable','Conflicting','Ambiguous'].includes(riskDecision)||!executionStatus||!executionCode||attempts===undefined||(attempts!==null&&(!Number.isInteger(attempts)||attempts<0))||(traceState==='available'&&attempts===null)||marketCollectedAtUtc===undefined||(marketCollectedAtUtc!==null&&Number.isNaN(Date.parse(marketCollectedAtUtc)))||marketDataVersion===undefined||!evidenceStateValid||!evidenceShapeValid)return null;return{traceId,schema,symbol,side,entryPrice,exitPrice,quantity,fees,feeBasis,feeRate,entrySlippageAmount,exitSlippageAmount,totalSlippageAmount,slippageBasis,fundingAmount,fundingBasis,netPnl,returnPct,outcome:outcome as RuntimeHistoricalPostTradeReview['outcome'],closedAtUtc,strategyId,strategyVersion,attributionBasis,traceState:traceState as RuntimeHistoricalPostTradeReview['traceState'],riskDecision:riskDecision as RuntimeHistoricalPostTradeReview['riskDecision'],executionStatus,executionCode,executionAttempts:attempts,marketCollectedAtUtc,marketDataVersion,evidenceState:evidenceState as RuntimeHistoricalPostTradeReview['evidenceState'],evidenceChain:chain}}
function normalizeHistoricalReconciliation(input:unknown):RuntimeHistoricalReconciliation|null{if(!input||typeof input!=='object')return null;const x=input as Record<string,unknown>;if('reportId'in x||'canonicalBytes'in x||'reasonCodes'in x||'localLegs'in x||'exchangeLegs'in x)return null;const kind=nonEmptyString(x.kind),traceId=nonEmptyString(x.traceId),schema=nonEmptyString(x.schema),observedAtUtc=nonEmptyString(x.observedAtUtc),evaluatedAtUtc=nonEmptyString(x.evaluatedAtUtc),state=nonEmptyString(x.state),canonicalSha256=nonEmptyString(x.canonicalSha256);if(!kind||!['position','protection','externalIsolation'].includes(kind)||!traceId||!/^reconciliation#[A-Fa-f0-9]{12}$/.test(traceId)||!schema||!observedAtUtc||!evaluatedAtUtc||Number.isNaN(Date.parse(observedAtUtc))||Number.isNaN(Date.parse(evaluatedAtUtc))||!state||typeof x.allowsRiskIncrease!=='boolean'||!canonicalSha256||!/^[A-Fa-f0-9]{64}$/.test(canonicalSha256))return null;return{kind:kind as RuntimeHistoricalReconciliation['kind'],traceId,schema,observedAtUtc,evaluatedAtUtc,state,allowsRiskIncrease:x.allowsRiskIncrease,canonicalSha256}}
...
export function normalizeHistoricalPageResponse(input:unknown):RuntimeHistoricalPageResponse|null{
  if(!input||typeof input!=='object')return null
  const x=input as Record<string,unknown>,contractVersion=nonEmptyString(x.contractVersion),requestId=nonEmptyString(x.requestId),kind=nonEmptyString(x.kind)
  if(contractVersion!=='1.0'||!requestId||!/^[A-Za-z0-9_-]{1,64}$/.test(requestId)||!kind||!['orders','equity','backtests','skillCalls','auditEvents','postTradeReviews','reconciliations'].includes(kind)||!x.page||typeof x.page!=='object')return null
  const page=x.page as Record<string,unknown>
  if(nonEmptyString(page.contractVersion)!=='1.0'||nonEmptyString(page.kind)!==kind)return null
  const typedKind=kind as RuntimeHistoricalPageKind
  const runtimePatch:Partial<WpeRuntimeState>=typedKind==='orders'?{historicalOrders:normalizeHistoricalCollection(page,normalizeHistoricalOrder)}
    :typedKind==='equity'?{historicalEquity:normalizeHistoricalCollection(page,normalizeHistoricalEquity)}
    :typedKind==='backtests'?{historicalBacktests:normalizeHistoricalCollection(page,normalizeBacktest)}
    :typedKind==='skillCalls'?{historicalSkillCalls:normalizeHistoricalCollection(page,normalizeSkillCall)}
    :typedKind==='auditEvents'?{historicalAuditEvents:normalizeHistoricalCollection(page,normalizeHistoricalAudit)}
    :typedKind==='postTradeReviews'?{historicalPostTradeReviews:normalizeHistoricalCollection(page,normalizeHistoricalPostTradeReview)}
    :{historicalReconciliations:normalizeHistoricalCollection(page,normalizeHistoricalReconciliation)}
  return{requestId,kind:typedKind,runtimePatch}
}
...
export function normalizeRuntimeEvent(input: unknown): WpeRuntimeState | null {
  ...
  const collections = ['account', 'positions', 'orders', 'risk', 'strategies', 'backtests', 'crossAssetResearch', 'distribution', 'telemetry', 'markets', 'publicMarkets', 'publicKlines', 'capabilities', 'llmGovernance', 'plugins', 'auditEvents', 'equityHistory', 'equityMarkets', 'equityBroker','historicalOrders','historicalEquity','historicalBacktests','historicalSkillCalls','historicalAuditEvents','historicalPostTradeReviews','historicalReconciliations', 'connectionStatus', 'strategyRegistry', 'strategyLifecycleEvents','skillCalls','memoryStatus','recentMemoryRetrievals','agentOperations','agentHandoffs','teacherLessons','teacherRecommendations','teacherCorrections','teacherOutcomes','notificationStatus','notificationOutbox','telegramSubscribers','authorizationMode','automaticExecutions','pendingApprovals','securityStorage']
  ...
  const historicalPostTradeReviews=normalizeHistoricalCollection(value.historicalPostTradeReviews,normalizeHistoricalPostTradeReview)
  const historicalReconciliations=normalizeHistoricalCollection(value.historicalReconciliations,normalizeHistoricalReconciliation)
  ...
  const state: WpeRuntimeState = {
    ...
    equityBroker,
    historicalOrders,historicalEquity,historicalBacktests,historicalSkillCalls,historicalAuditEvents,historicalPostTradeReviews,historicalReconciliations,
    connectionStatus,
    diagnostic,
    memoryStatus,
