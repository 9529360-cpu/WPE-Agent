...
export type RuntimeHistoricalPostTradeReview={schema:string;clientOrderId:string;cycleId:string;symbol:string;side:string;entryPrice:number;exitPrice:number;quantity:number;fees:number;feeBasis:string;feeRate:number;entrySlippageAmount:number;exitSlippageAmount:number;totalSlippageAmount:number;slippageBasis:string;fundingAmount:number;fundingBasis:string;netPnl:number;returnPct:number;outcome:'win'|'loss'|'flat';closedAtUtc:string;strategyId:string|null;strategyVersion:string;attributionBasis:string}
...
function normalizeHistoricalPostTradeReview(input:unknown):RuntimeHistoricalPostTradeReview|null{if(!input||typeof input!=='object')return null;const x=input as Record<string,unknown>,schema=nonEmptyString(x.schema),clientOrderId=nonEmptyString(x.clientOrderId),cycleId=nonEmptyString(x.cycleId),symbol=nonEmptyString(x.symbol),side=nonEmptyString(x.side),feeBasis=nonEmptyString(x.feeBasis),slippageBasis=nonEmptyString(x.slippageBasis),fundingBasis=nonEmptyString(x.fundingBasis),outcome=nonEmptyString(x.outcome),closedAtUtc=nonEmptyString(x.closedAtUtc),strategyVersion=nonEmptyString(x.strategyVersion),attributionBasis=nonEmptyString(x.attributionBasis),strategyId=x.strategyId===null?null:nonEmptyString(x.strategyId);const entryPrice=finiteNumber(x.entryPrice),exitPrice=finiteNumber(x.exitPrice),quantity=finiteNumber(x.quantity),fees=finiteNumber(x.fees),feeRate=finiteNumber(x.feeRate),entrySlippageAmount=finiteNumber(x.entrySlippageAmount),exitSlippageAmount=finiteNumber(x.exitSlippageAmount),totalSlippageAmount=finiteNumber(x.totalSlippageAmount),fundingAmount=finiteNumber(x.fundingAmount),netPnl=finiteNumber(x.netPnl),returnPct=finiteNumber(x.returnPct);if(schema!=='wpe.post-trade-review/1.4'||!clientOrderId||!cycleId||!symbol||!side||entryPrice===null||entryPrice<=0||exitPrice===null||exitPrice<=0||quantity===null||quantity<=0||fees===null||fees<0||!feeBasis||feeRate===null||feeRate<0||entrySlippageAmount===null||exitSlippageAmount===null||totalSlippageAmount===null||!slippageBasis||fundingAmount===null||!fundingBasis||netPnl===null||returnPct===null||!outcome||!['win','loss','flat'].includes(outcome)||!closedAtUtc||Number.isNaN(Date.parse(closedAtUtc))||strategyId===undefined||!strategyVersion||!attributionBasis)return null;return{schema,clientOrderId,cycleId,symbol,side,entryPrice,exitPrice,quantity,fees,feeBasis,feeRate,entrySlippageAmount,exitSlippageAmount,totalSlippageAmount,slippageBasis,fundingAmount,fundingBasis,netPnl,returnPct,outcome:outcome as RuntimeHistoricalPostTradeReview['outcome'],closedAtUtc,strategyId,strategyVersion,attributionBasis}}
