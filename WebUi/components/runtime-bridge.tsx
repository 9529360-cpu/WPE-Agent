...
export type WpeRuntimeState = {
  ...
  historicalExecutionReality?:RuntimeHistoricalCollection<RuntimeHistoricalExecutionReality>
}
...
export type RuntimeHistoricalExecutionReality={observedAtUtc:string;strategyId:string;strategyVersion:string;costModelVersion:string;symbol:string;state:string;terminal:boolean;priceComparable:boolean;feeComparable:boolean;totalComparable:boolean;fillRatio:number;slippageDriftBps:number|null;feeDriftBps:number|null;totalExecutionDriftBps:number|null;observationLatencyMs:number;reasonCode:string}
...
function normalizeHistoricalExecutionReality(input:unknown):RuntimeHistoricalExecutionReality|null{if(!input||typeof input!=='object')return null;const x=input as Record<string,unknown>,observedAtUtc=nonEmptyString(x.observedAtUtc),strategyId=nonEmptyString(x.strategyId),strategyVersion=nonEmptyString(x.strategyVersion),costModelVersion=nonEmptyString(x.costModelVersion),symbol=nonEmptyString(x.symbol),state=nonEmptyString(x.state),fillRatio=finiteNumber(x.fillRatio),slippageDriftBps=x.slippageDriftBps===null?null:finiteNumber(x.slippageDriftBps),observationLatencyMs=finiteNumber(x.observationLatencyMs),reasonCode=nonEmptyString(x.reasonCode),feeDriftBps=x.feeDriftBps===null?null:finiteNumber(x.feeDriftBps),totalExecutionDriftBps=x.totalExecutionDriftBps===null?null:finiteNumber(x.totalExecutionDriftBps);if(!observedAtUtc||Number.isNaN(Date.parse(observedAtUtc))||!strategyId||!strategyVersion||!costModelVersion||!symbol||!state||typeof x.terminal!=='boolean'||typeof x.priceComparable!=='boolean'||typeof x.feeComparable!=='boolean'||typeof x.totalComparable!=='boolean'||fillRatio===null||fillRatio<0||fillRatio>1||observationLatencyMs===null||!Number.isInteger(observationLatencyMs)||observationLatencyMs<0||!reasonCode||(x.priceComparable!==true&&slippageDriftBps!==null)||(x.priceComparable===true&&slippageDriftBps===null)||(x.feeComparable!==true&&feeDriftBps!==null)||(x.feeComparable===true&&feeDriftBps===null)||(x.totalComparable!==true&&totalExecutionDriftBps!==null)||(x.totalComparable===true&&totalExecutionDriftBps===null))return null;return{observedAtUtc,strategyId,strategyVersion,costModelVersion,symbol,state,terminal:x.terminal,priceComparable:x.priceComparable,feeComparable:x.feeComparable,totalComparable:x.totalComparable,fillRatio,slippageDriftBps,feeDriftBps,totalExecutionDriftBps,observationLatencyMs,reasonCode}}
...
export function normalizeRuntimeEvent(input: unknown): WpeRuntimeState | null {
  ...
  const collections = ['account', 'positions', 'orders', 'risk', 'strategies', 'backtests', 'crossAssetResearch', 'distribution', 'telemetry', 'markets', 'publicMarkets', 'publicKlines', 'capabilities', 'llmGovernance', 'plugins', 'auditEvents', 'equityHistory', 'equityMarkets', 'equityBroker','historicalOrders','historicalEquity','historicalBacktests','historicalSkillCalls','historicalAuditEvents','historicalExecutionReality', 'connectionStatus', 'strategyRegistry', 'strategyLifecycleEvents','skillCalls','memoryStatus','recentMemoryRetrievals','agentOperations','agentHandoffs','teacherLessons','teacherRecommendations','teacherCorrections','teacherOutcomes','notificationStatus','notificationOutbox','telegramSubscribers','authorizationMode','automaticExecutions','pendingApprovals','securityStorage']
  ...
  const historicalExecutionReality=normalizeHistoricalCollection(value.historicalExecutionReality,normalizeHistoricalExecutionReality)
  ...
  const state: WpeRuntimeState = {
    ...
    equityBroker,
    historicalOrders,historicalEquity,historicalBacktests,historicalSkillCalls,historicalAuditEvents,historicalExecutionReality,
    connectionStatus,
    diagnostic,
    memoryStatus,
