...
test('maps all seven normalized top-level collections to real available rows', () => {
  ...
  const runtime: WpeRuntimeState = {
    ...
    historicalPostTradeReviews: collection([{ traceId: 'trade#ABCDEF123456', schema: 'wpe.post-trade-review/1.4', symbol: 'SOLUSDT', side: 'Long', entryPrice: 145, exitPrice: 150, quantity: 2, fees: .12, feeBasis: 'exchange-reported-usdt', feeRate: 0, entrySlippageAmount: .01, exitSlippageAmount: .02, totalSlippageAmount: .03, slippageBasis: 'intent-expected-vs-fill', fundingAmount: .5, fundingBasis: 'exchange-reported-window', netPnl: 10.38, returnPct: .0358, outcome: 'win', closedAtUtc: '2026-07-22T01:05:00Z', strategyId: 'mean-reversion-sol', strategyVersion: '3.0.0', attributionBasis: 'automatic-artifact', traceState: 'available', riskDecision: 'Approved', executionStatus: 'Succeeded', executionCode: 'automatic.succeeded', executionAttempts: 1, marketCollectedAtUtc: '2026-07-22T01:04:30Z', marketDataVersion: 'provider-market-v1', ...({ clientOrderId: secret, cycleId: secret, executionId: secret } as object) }]),
  ...
  for (const value of ['BTCUSDT', 'binance-futures', 'trend-alpha', 'market-research', 'risk-gate', 'mean-reversion-sol', 'exchange-reported-usdt', 'trade#ABCDEF123456', 'Approved', 'Succeeded', 'automatic.succeeded', 'provider-market-v1', 'reconciliation#ABCDEF123456', 'Confirmed', 'Allowed at audit time']) assert.match(html, new RegExp(value))
