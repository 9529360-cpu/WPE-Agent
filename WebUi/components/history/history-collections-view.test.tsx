...
test('maps all seven normalized top-level collections to real available rows', () => {
  ...
  const runtime: WpeRuntimeState = {
    ...
    historicalAuditEvents: collection([{ id: 'audit-real-1', occurredAtUtc: '2026-07-22T01:04:00Z', category: 'risk', source: 'risk-gate', correlationId: 'correlation#ABCDEF123456', status: 'Blocked', ...({ rawCorrelationId: secret, payload: secret } as object) }]),
    historicalPostTradeReviews: collection([{ traceId: 'trade#ABCDEF123456', schema: 'wpe.post-trade-review/1.4', symbol: 'SOLUSDT', side: 'Long', entryPrice: 145, exitPrice: 150, quantity: 2, fees: .12, feeBasis: 'exchange-reported-usdt', feeRate: 0, entrySlippageAmount: .01, exitSlippageAmount: .02, totalSlippageAmount: .03, slippageBasis: 'intent-expected-vs-fill', fundingAmount: .5, fundingBasis: 'exchange-reported-window', netPnl: 10.38, returnPct: .0358, outcome: 'win', closedAtUtc: '2026-07-22T01:05:00Z', strategyId: 'mean-reversion-sol', strategyVersion: '3.0.0', attributionBasis: 'automatic-artifact', traceState: 'available', riskDecision: 'Approved', executionStatus: 'Succeeded', executionCode: 'automatic.succeeded', executionAttempts: 1, marketCollectedAtUtc: '2026-07-22T01:04:30Z', marketDataVersion: 'provider-market-v1', evidenceState: 'available', evidenceChain: [{ stage: 'market', status: 'succeeded', canonicalSha256: 'b'.repeat(64), asOfUtc: '2026-07-22T01:04:00Z' }, { stage: 'research', status: 'succeeded', canonicalSha256: 'c'.repeat(64), asOfUtc: '2026-07-22T01:04:05Z' }, { stage: 'strategy', status: 'succeeded', canonicalSha256: 'd'.repeat(64), asOfUtc: '2026-07-22T01:04:10Z' }, { stage: 'risk', status: 'succeeded', canonicalSha256: 'e'.repeat(64), asOfUtc: '2026-07-22T01:04:15Z' }], ...({ clientOrderId: secret, cycleId: secret, executionId: secret, canonicalBytes: secret } as object) }]),
    historicalReconciliations: collection([{ kind: 'position', traceId: 'reconciliation#ABCDEF123456', schema: 'wpe.position-reconciliation/1.0', observedAtUtc: '2026-07-22T01:06:00Z', evaluatedAtUtc: '2026-07-22T01:06:00Z', state: 'Confirmed', allowsRiskIncrease: true, canonicalSha256: 'a'.repeat(64), ...({ reportId: secret } as object) }]),
  }

  const html = render(projectRuntimeHistory(runtime))
  for (const value of ['BTCUSDT', 'binance-futures', 'trend-alpha', 'market-research', 'risk-gate', 'mean-reversion-sol', 'exchange-reported-usdt', 'trade#ABCDEF123456', 'Approved', 'Succeeded', 'automatic.succeeded', 'provider-market-v1', 'Evidence: available', '4/4 canonical stages', 'strategy: succeeded', 'dddddddddddd', 'reconciliation#ABCDEF123456', 'Confirmed', 'Allowed at audit time']) assert.match(html, new RegExp(value))
