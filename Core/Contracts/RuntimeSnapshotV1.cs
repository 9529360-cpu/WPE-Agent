...
public sealed record RuntimeNotificationWorkerHealthV1(string State,int ConsecutiveFailures,DateTimeOffset? LastSuccessAtUtc,DateTimeOffset? LastFailureAtUtc,DateTimeOffset? NextAttemptAtUtc,string? DiagnosticCode,bool OwnsSingleInstanceLease,DateTimeOffset UpdatedAtUtc);
public sealed record RuntimeNotificationStatusV1(bool Enabled,bool TelegramStored,bool TelegramReady,bool WhatsAppStored,bool WhatsAppReady,bool LegacyMigrationPending,string? LegacyMigrationDiagnosticCode,IReadOnlyList<string> EventKinds,bool QuietHoursEnabled,string QuietHoursStart,string QuietHoursEnd,string QuietHoursTimeZone,int PendingCount,int RetryingCount,int SentCount,int DeadLetterCount,RuntimeNotificationWorkerHealthV1? TelegramPoller=null,RuntimeNotificationWorkerHealthV1? TelegramDispatcher=null);
