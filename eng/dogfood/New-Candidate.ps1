...
if([string]$verification.packageTreeSha256 -notmatch '^[a-f0-9]{64}$'){throw 'package.verification-tree-binding-missing'}
try{$verifiedPackageFileCount=[int]$verification.packageFileCount}catch{throw 'package.verification-tree-binding-missing'}
if($verifiedPackageFileCount -le 0){throw 'package.verification-tree-binding-missing'}
$sourceSnapshot=@(Get-TreeSnapshot $source)
$sourceTreeSha256=Get-TextSha256 ($sourceSnapshot -join [Environment]::NewLine)
if($verifiedPackageFileCount -ne $sourceSnapshot.Count -or
   [string]$verification.packageTreeSha256 -ne $sourceTreeSha256){
    throw 'package.verification-source-mismatch'
}
