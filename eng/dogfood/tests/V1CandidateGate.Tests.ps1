...
function Assert-Throws([string]$Code, [scriptblock]$Action) {
    ...
    catch { if ($_.Exception.Message -cne $Code) { throw "expected $Code, got $($_.Exception.Message)" } }
}
...
try {
    ...
    $candidateManifest = [ordered]@{
        ...
        version='1.0.0'
        sourceIdentity='commit/candidate'
        environment='Testnet'
        configurationSchema='cfg/1'
        migrationVersion='db/1'
        createdUtc=[DateTimeOffset]::UtcNow.AddMinutes(-2).ToString('O')
        files=@()
        gates=@('build','tests')
    }
    $lkgManifest = [ordered]@{
        schemaVersion='wpe.dogfood-release-manifest/1.1'
        version='0.9.0'
        sourceIdentity='commit/lkg'
        environment='Testnet'
        configurationSchema='cfg/1'
        migrationVersion='db/1'
        createdUtc=[DateTimeOffset]::UtcNow.AddMinutes(-3).ToString('O')
        files=@()
        gates=@('build','tests')
    }
    $candidateHash = Write-JsonFixture (Join-Path $candidate.FullName 'release-manifest.json') $candidateManifest
    $lkgHash = Write-JsonFixture (Join-Path $lkg.FullName 'release-manifest.json') $lkgManifest

    $runtimePath = Join-Path $root 'runtime.json'
    function Write-RuntimeProof([hashtable]$Overrides) {
        $now=[DateTimeOffset]::UtcNow
        $proof=[ordered]@{
            schemaVersion='wpe.trusted-runtime-proof/1.1'
            proofId='runtime-1'
            sourceIdentity='trusted-bridge'
            sourceTimestampUtc=$now.AddSeconds(-2).ToString('O')
            snapshotTimestampUtc=$now.ToString('O')
            environment='Testnet'
            ...
            evidenceGates=@(
                [ordered]@{reference='authority';status='passed'},
                [ordered]@{reference='freshness';status='passed'}
            )
            candidateManifestSha256=$candidateHash
            lastKnownGoodManifestSha256=$lkgHash
            probes=[ordered]@{
        ...
        Write-JsonFixture $runtimePath $proof
    }
    $runtimeHash = Write-RuntimeProof @{}
    ...
    $providerHash = Write-JsonFixture $providerPath ([ordered]@{
    ...
    $base = @{
        CandidateRoot=$candidate.FullName
        ExpectedCandidateManifestHash=$candidateHash
        LastKnownGoodRoot=$lkg.FullName
        ExpectedLastKnownGoodManifestHash=$lkgHash
        TrustedRuntimeProofPath=$runtimePath
        ExpectedTrustedRuntimeProofHash=$runtimeHash
        ExpectedRuntimeProofId='runtime-1'
        ExpectedRuntimeSourceIdentity='trusted-bridge'
        ExpectedEvidenceGateRefs=@('authority','freshness')
        SoakEvidencePath=$soakPath
        ExpectedSoakEvidenceHash=$soakHash
        MinimumSoakDurationMinutes=1440
        FullReleaseEvidencePath=$suitePath
        ExpectedFullReleaseEvidenceHash=$suiteHash
        ProducerVerdictPaths=$verdictPaths
        ExpectedProducerVerdictHashes=$verdictHashes
        AuthorizedProviderFixtureRoot=$providerRoot.FullName
        ProviderFixturePath=$providerPath
        ExpectedProviderFixtureHash=$providerHash
        DryRun=$true
        RejectMainnet=$true
    }
    ...
    $oldRuntimeHash = Write-JsonFixture $runtimePath ([ordered]@{
        proofId='runtime-1'
        sourceIdentity='trusted-bridge'
        status='passed'
        fresh=$true
        environment='Testnet'
        gateEvidenceRefs=@('authority','freshness')
    })
    $oldRuntime = $base.Clone()
    $oldRuntime.ExpectedTrustedRuntimeProofHash = $oldRuntimeHash
    Assert-Throws 'runtime.proof-unknown-field' { & $scriptPath @oldRuntime }

    $badBindingHash = Write-RuntimeProof @{candidateManifestSha256=('0'*64)}
    $badBinding = $base.Clone()
    $badBinding.ExpectedTrustedRuntimeProofHash = $badBindingHash
    Assert-Throws 'runtime.proof-manifest-binding-mismatch' { & $scriptPath @badBinding }

    $old=[DateTimeOffset]::UtcNow.AddMinutes(-10)
    $staleHash = Write-RuntimeProof @{
        sourceTimestampUtc=$old.AddSeconds(-2).ToString('O')
        snapshotTimestampUtc=$old.ToString('O')
    }
    $stale = $base.Clone()
    $stale.ExpectedTrustedRuntimeProofHash = $staleHash
    Assert-Throws 'runtime.proof-stale' { & $scriptPath @stale }

    $missingProbeHash = Write-RuntimeProof @{
        probes=[ordered]@{
            install=[ordered]@{status='passed';target='candidate';evidenceSha256=('1'*64)}
            startup=[ordered]@{status='passed';target='candidate';evidenceSha256=('2'*64)}
            restartRecovery=[ordered]@{status='passed';target='candidate';evidenceSha256=('3'*64)}
    ...
    $missingProbe = $base.Clone()
    $missingProbe.ExpectedTrustedRuntimeProofHash = $missingProbeHash
    Assert-Throws 'runtime.probes-invalid' { & $scriptPath @missingProbe }

    $runtimeHash = Write-RuntimeProof @{}
    $base.ExpectedTrustedRuntimeProofHash = $runtimeHash

    $relativeFixture = $base.Clone()
    $relativeFixture.ProviderFixturePath = 'provider-read-only.json'
    Assert-Throws 'provider.fixture-path.not-absolute' { & $scriptPath @relativeFixture }
    $uncFixture = $base.Clone()
    $uncFixture.ProviderFixturePath = '\\server\share\provider.json'
    Assert-Throws 'provider.fixture-path.network-path-forbidden' { & $scriptPath @uncFixture }
    $uncRoot = $base.Clone()
    $uncRoot.AuthorizedProviderFixtureRoot = '\\server\share\fixtures'
    ...
    $outsideFixture = $base.Clone()
    $outsideFixture.ProviderFixturePath = $outsidePath
    ...
    $siblingFixture = $base.Clone()
    $siblingFixture.ProviderFixturePath = $siblingPath
    Assert-Throws 'provider.fixture-outside-authorized-root' { & $scriptPath @siblingFixture }

    $traversalFixture = $base.Clone()
    $traversalFixture.ProviderFixturePath = Join-Path $providerRoot.FullName '..\outside-provider.json'
    Assert-Throws 'provider.fixture-outside-authorized-root' { & $scriptPath @traversalFixture }

    $missingFixture = $base.Clone()
    $missingFixture.ProviderFixturePath = (Join-Path $providerRoot.FullName 'missing-provider.json')
    Assert-Throws 'provider.fixture-missing' { & $scriptPath @missingFixture }
    $mismatchFixture = $base.Clone()
    $mismatchFixture.ExpectedProviderFixtureHash = ('0' * 64)
    ...
    try {
        ...
        $reparseFixture = $base.Clone()
        $reparseFixture.ProviderFixturePath = Join-Path $junction 'provider.json'
        Assert-Throws 'provider.fixture-reparse-forbidden' { & $scriptPath @reparseFixture }
    } catch {
        if ($_.Exception.Message -ne 'provider.fixture-reparse-forbidden') {
            Write-Output 'V1CandidateGate.Tests: reparse regression SKIP (junction unavailable)'
        }
    }

    $failedSuiteHash = Write-JsonFixture $suitePath ([ordered]@{ status='failed'; configuration='Release'; total=1522; failed=3 })
    $failed = $base.Clone()
    $failed.ExpectedFullReleaseEvidenceHash = $failedSuiteHash
    Assert-Throws 'full-release.not-passed' { & $scriptPath @failed }

    $missing = $base.Clone()
    $missing.FullReleaseEvidencePath = (Join-Path $root 'missing.json')
    Assert-Throws 'full-release.missing' { & $scriptPath @missing }
    $badHash = $base.Clone()
    $badHash.ExpectedFullReleaseEvidenceHash = ('0' * 64)
    ...
    $mutationMissing = $base.Clone()
    $mutationMissing.TestnetMutationSmoke = $true
    ...
    $rejected = $base.Clone()
    $rejected.ExpectedProducerVerdictHashes = @($rejectedHash,$verdictHashes[1],$verdictHashes[2])
    ...
    foreach ($forbidden in @('Invoke-WebRequest','Invoke-RestMethod','Start-Process')) {
        if ($source -match [regex]::Escape($forbidden)) { throw "forbidden command $forbidden" }
    }
    if ($source.IndexOf('release-manifest.json',[StringComparison]::Ordinal) -lt 0) { throw 'release manifest contract missing' }
    if ($source.IndexOf('wpe.trusted-runtime-proof/1.1',[StringComparison]::Ordinal) -lt 0) { throw 'runtime proof v1.1 contract missing' }

    Write-Output 'V1CandidateGate.Tests: PASS'
}
finally {
