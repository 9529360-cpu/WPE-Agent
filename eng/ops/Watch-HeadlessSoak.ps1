...
function Utc([DateTimeOffset]$value) {
    ...
    while ([DateTimeOffset]::UtcNow -lt $deadline) {
        ...
        try {
            ...
            try {
                ...
                throw [InvalidDataException]::new('health.timestamp-invalid')
            }

            $futureTolerance = [TimeSpan]::FromSeconds(5)
            if ($parsedObserved -gt $sampledAt.Add($futureTolerance)) {
                throw [InvalidDataException]::new('health.future-observation')
            }
            $age = [Math]::Max(0.0, ($sampledAt - $parsedObserved).TotalSeconds)
            if ($age -gt $MaximumHealthAgeSeconds) {
                throw [InvalidDataException]::new('health.stale')
            }

            $processState = [string]$health.state
            $processCode = [string]$health.code
            if ($allowedStates -notcontains $processState) {
                throw [InvalidDataException]::new('health.state-invalid')
            }
            if ([string]::IsNullOrWhiteSpace($processCode) -or $processCode.Length -gt 128) {
                throw [InvalidDataException]::new('health.code-invalid')
            }
            ...
            $healthError = if ($_.Exception.Message -like 'health.*') {
                $_.Exception.Message
            } else {
                'health.unavailable-or-malformed'
            }
            if ($inGrace -and $healthError -eq 'health.unavailable') {
                $graceSamples++
                $reason = 'soak.startup-grace'
                $accepted = $true
            } else {
...
$status = if ($sampleCount -ge $expectedMinimumSamples -and $unhealthySamples -eq 0 -and $invalidSamples -eq 0) {
    'passed'
} else {
    'failed'
}
