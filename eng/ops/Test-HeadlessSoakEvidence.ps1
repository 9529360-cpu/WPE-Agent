...
if(-not [string]::IsNullOrWhiteSpace($ExpectedEvidenceSha256) -and
   $evidenceSnapshot.Hash -ne $ExpectedEvidenceSha256.ToLowerInvariant()){throw 'soak.evidence-hash-mismatch'}
try{$evidence=(Convert-SnapshotToText $evidenceSnapshot)|ConvertFrom-Json}catch{throw 'soak.evidence-malformed'}
$allowed=@(
...
while($null -ne ($sampleLine=$sampleReader.ReadLine())){
    ...
    try{$sample=$sampleLine|ConvertFrom-Json}catch{throw 'soak.sample-malformed'}
