...
if (-not (Test-Path -LiteralPath $publish -PathType Container)) { throw "Signing staging directory is missing." }
...
$executables = @(Get-ChildItem -LiteralPath $publish -File -Filter "*.exe")
if ($executables.Count -ne 1) { throw "Expected exactly one application executable in signing staging." }
& $signTool sign /fd SHA256 /td SHA256 /tr $TimestampUrl /sha1 $certificate.Thumbprint /s My $executables[0].FullName
if ($LASTEXITCODE -ne 0) { throw "Authenticode signing failed." }
& $signTool verify /pa /all /v $executables[0].FullName
if ($LASTEXITCODE -ne 0) { throw "Authenticode verification failed." }
$signature = Get-AuthenticodeSignature -LiteralPath $executables[0].FullName
if ($signature.Status -ne [System.Management.Automation.SignatureStatus]::Valid) { throw "PowerShell did not validate the resulting Authenticode signature." }
Write-Host "Signing and verification passed: $($executables[0].FullName)" -ForegroundColor Green
